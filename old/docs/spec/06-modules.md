...
`self` 只在 class 方法和 `check` 块内有效，其他位置使用 `self` 是错误。`self` 不可重新绑定，`self = x` 是编译期错误。
...
4. **运行时变量初始化**：按文件声明顺序（从上到下）初始化顶层 `var`，此后模块进入可调用状态
...
- 按文件内的声明顺序从上到下依次初始化
- 引用尚未初始化的同模块顶层 `var` 是**加载期错误**，可静态检查
- 跨模块引用：被导入模块的 `var` 在本模块 `var` 初始化前已完成初始化（模块按依赖拓扑序加载）

### 各阶段可读写汇总

| | 阶段 1–2（声明收集与解析） | 阶段 3（配置求值） | 阶段 4（var 初始化） | 运行期 |
|---|---|---|---|---|
| `env` | 可读 | 可读 | 可读 | 可读 |
| `host` | 错误 | 错误 | 可读写 | 可读写 |
| 配置 | 待求值 | 按拓扑序求值 | 只读 | 只读 |
