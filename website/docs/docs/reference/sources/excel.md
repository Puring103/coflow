...
Excel 与 CSV、飞书/Lark 共享 [表格 Source](./table.md) 规则：第一行表头、`id` 列作为 key、`sheets` 映射、`columns` 映射、`#` 控制列和 `@expand`。
...
写回失败使用 `EXCEL-WRITE` 诊断。读取和解析阶段的诊断见 [错误码](../diagnostics/codes.md)。
