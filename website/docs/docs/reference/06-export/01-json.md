...
JSON 导出产物是一个不可变 generation 目录。每个有记录的非 `abstract` CFT type 导出为一个文件：
...
`outputs.data.dir` 或 `--out` 是 generation 的放置锚点，不再是会被原地替换的目录。Coflow 在其同级位置写入并校验新的不可变 generation；完成后通过项目目录下 `.coflow/artifacts/active.json` 一次激活 data、code 和 `@idAsEnum` lock state。命令成功信息会输出本次激活的实际 generation 目录。

消费者应使用命令返回的目录，或读取 active manifest 中 `outputs.data.generation_dir`。不要修改 generation 中的文件；后续发布会创建新 generation，旧 generation 保持不变。
