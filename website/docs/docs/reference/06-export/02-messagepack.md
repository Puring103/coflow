...
MessagePack 导出产物是一个目录。每个非 `abstract` CFT type 导出为一个文件：
...
`coflow export messagepack` 和 `coflow build` 写入 MessagePack 时，会完整接管输出目录。写入先进入同级 staging 目录，全部成功后再替换目标目录。

不要把手写文件放进 MessagePack 输出目录。
