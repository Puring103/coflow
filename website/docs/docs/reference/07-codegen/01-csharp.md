...
`Coflow.Cfd.Runtime` 负责 CFD tokenizer/parser、span、文本读取、引用缓存和资源限制；生成 binding 为每个可实例化类型生成显式构造函数调用，并使用 `CfdValueReader` 完成值转换。生成代码不使用反射，runtime 不加载 CFT，也不会根据目录或文件名猜测类型。
