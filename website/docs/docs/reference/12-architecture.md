...
artifact release plan 保存 generation-bound session、provider 和 decoded options。prepare 阶段
完成所有 safety validation 与纯内存 provider generation，publish 阶段才接触 staging 和
...
document 的外部修改也会推进 validation revision。CFD source、canonical AST、syntax
