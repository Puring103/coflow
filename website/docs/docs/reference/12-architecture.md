...
- 验证并封存全部不可变 artifact generation，事务性替换稳定输出目录，再以一次 publication 原子发布 active manifest。

active manifest 同时选择 data、code generation 和 `@idAsEnum` lock state。它的发布属于 CLI 宿主职责，不放进 runtime；所有可报告的文件系统操作都在最后一次 manifest 原子替换前完成。稳定输出目录使用同级 staging 和 backup 事务性替换；后续步骤失败时恢复旧目录并保持旧 manifest。根目录 `coflow.enum.lock.json` 是激活前写好的版本化镜像，不参与 active snapshot 选择；若最终激活失败，它可以暂时领先于 active manifest，但不会成为运行时状态。
...
- 事务性替换 `outputs.*.dir` 指定的稳定输出目录。
- 只通过一个 active manifest 原子激活完整 snapshot。

这样可以避免半生成目录被消费者误读；任一文件系统操作失败时，稳定输出和 active state 都会保留旧的完整版本。artifact preflight 还会解析已存在祖先的真实路径，避免输出经 symlink/junction 落进项目根、schema 或 source 目录。
