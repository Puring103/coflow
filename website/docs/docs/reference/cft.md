...
- 内建函数名：`len`、`contains`、`unique`、`min`、`max`、`sum`、`keys`、`values`、`matches`
...
注解用于补充 schema 语义，影响加载器、编辑器展示、导出和代码生成。注解写在 `type`、`enum`、字段或 enum variant 之前。

| 注解 | 适用目标 | 影响阶段 | 说明 |
| --- | --- | --- | --- |
| `@display("text")` | type、enum、field、enum variant | editor / codegen | 可读名称，用于编辑器展示和 codegen 注释 |
...
- 空值、`_`、`null`、数组、字典、内联对象、记录引用等值语法见 [单元格值语法](./sources/cell-value.md)。
- CFD 文本配置语法见 [CFD 语法参考](./cfd.md)。
