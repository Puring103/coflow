...
构建校验按配置的变体执行。变体值缺失或显式为 `None` 时，使用基础字段值执行校验；基础值仍须满足对应的校验规则。

维度记录类型必须与目标字段对应的辅助类型一致。重复记录、未知变体和值类型错误会报告诊断。目标基础记录不存在时，残留的维度记录会被静默忽略；下次运行 `coflow build` 时，维度文件按现有基础记录更新并清理残留。
