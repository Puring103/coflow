...
public sealed class CfdLoadContext
{
    ...
    private readonly Stack<(string DeclaredType, int FunctionStart, string ValuePath, CoflowValueId ValueId)> _currentValues = new();
    private readonly Dictionary<(string DeclaredType, string Key), CoflowValueId> _recordIds = new();
    private readonly IReadOnlyDictionary<CfdRecordNode, string> _recordPaths;
    private readonly IReadOnlyDictionary<CfdRecordNode, long> _recordModuleIds;
    ...
    private readonly List<CoflowLoadedValue> _values = new();
        ...
        uint snapshotId = 0)
    {
        ...
        _recordModuleIds = bound.Documents.SelectMany(document => document.Records.Select(record =>
            (record, document.ModuleId))).ToDictionary(pair => pair.record, pair => pair.ModuleId);
    ...
    internal IReadOnlyList<CoflowLoadedValue> Values => _values;
    internal uint Generation => _generation;
    internal uint SnapshotId => _snapshotId;
    ...
    public void BindHostFunction(CoflowHostFunctionBinding binding)
    {
        binding.Entry.ConfigureHost(binding.Implementation, binding.Call);
    }

    internal object AttachRecordValue(ICoflowTypeMetadata metadata, string key, object value)
    {
        var id = AllocateValueId();
        var identified = metadata.WithValueId(value, id);
        _recordIds.Add((metadata.DeclaredType, key), id);
        _values.Add(new CoflowLoadedValue(metadata.TypeId, metadata.DeclaredType, key, identified));
        return identified;
    }

    internal void CompleteRecordValue(string declaredType, string key)
    {
        var value = _values.Single(item => item.DeclaredType == declaredType && item.RecordKey == key);
        foreach (var function in _functions.Where(entry =>
                     entry.Identity.DeclaredType == declaredType &&
                     entry.Identity.RecordKey == key))
            function.Owner = value.ApiValue;
        value.Functions = _functions.Where(entry =>
            entry.Identity.DeclaredType == declaredType &&
            entry.Identity.RecordKey == key &&
            entry.Identity.ValuePath == entry.Identity.FieldName).ToArray();
    }

    public T ReadValue<T>(string declaredType, CfdValueNode node, Func<T> factory)
    {
        if (declaredType is null) throw new ArgumentNullException(nameof(declaredType));
        if (node is null) throw new ArgumentNullException(nameof(node));
        if (factory is null) throw new ArgumentNullException(nameof(factory));
        var start = _functions.Count;
        var id = AllocateValueId();
        var valuePath = LocateFunctionValue(node)?.ValuePath ?? declaredType;
        _currentValues.Push((declaredType, start, valuePath, id));
        try
        {
            var value = factory();
            var metadata = (ICoflowTypeMetadata)Bindings[declaredType];
            var identified = metadata.WithValueId(value!, id);
            var functions = _functions.Skip(start).Where(entry => entry.Owner is null).ToArray();
            var boxed = (object)identified;
            foreach (var function in functions) function.Owner = boxed;
            _values.Add(new CoflowLoadedValue(metadata.TypeId, declaredType, CurrentRecordKey ?? string.Empty, boxed)
            {
                Functions = functions,
            });
            return (T)identified;
    ...
    internal CoflowFunctionEntry ResolveFunction(object owner, string declaredType, string fieldName) =>
        _functions.First(entry =>
            ...
            entry.Identity.DeclaredType == declaredType &&
            entry.Identity.FieldName == fieldName);

    internal CoflowFunctionEntry ResolveFunction(string declaredType, string recordKey, string fieldName) =>
        _functions.First(entry =>
            entry.Identity.DeclaredType == declaredType &&
            entry.Identity.RecordKey == recordKey &&
            entry.Identity.FieldName == fieldName &&
            entry.Identity.ValuePath == fieldName);
        ...
        params Type[] parameterTypes)
    {
        ...
        return CreateFunctionEntry(node as CfdFunctionValue, fieldName, fieldName,
            resultType, parameterTypes, requiresCfdBody: false, isDefault: false);
    }
        ...
        params Type[] parameterTypes)
    {
        if (node is not null && node is not CfdFunctionValue)
            throw new CfdLoadException(new[] { new CfdDiagnostic(
                ...
                node.Span) });
        return CreateFunctionEntry(node as CfdFunctionValue, fieldName, fieldName,
            resultType, parameterTypes, requiresCfdBody: true, isDefault: false);
    }
        ...
        params Type[] parameterTypes)
    {
        ...
        if (node is not CfdFunctionValue function)
            throw new InvalidOperationException("A schema function default must contain a function value.");
        var valuePath = _currentValues.Count == 0
            ? fieldName
            : $"{_currentValues.Peek().ValuePath}.{fieldName}";
        return CreateFunctionEntry(function, fieldName, valuePath,
            resultType, parameterTypes, requiresCfdBody: true, isDefault: true);
    }

    public TFunction FunctionValue<TFunction>(
        CfdValueNode node,
        Type resultType,
        Type[] parameterTypes)
    {
        ...
        var entry = CreateFunctionEntry(source, location.FieldName, location.ValuePath,
            resultType, parameterTypes, requiresCfdBody: true, isDefault: false);
        var environmentId = _currentValues.Count != 0
            ? _currentValues.Peek().ValueId
            : _recordIds[(CurrentRecordType!, CurrentRecordKey ?? string.Empty)];
        return CoflowFunctionHandle.Create<TFunction>(
            new CoflowFunctionId(_snapshotId, CoflowFunctionKind.Program, entry.TargetIndex),
            environmentId);
    }
    ...
    private CoflowFunctionEntry CreateFunctionEntry(
        ...
        bool isDefault)
    {
        var declaredType = CurrentValueType ?? CurrentRecordType ?? throw new InvalidOperationException(
        ...
        var entry = new CoflowFunctionEntry(
            ...
            ((ICoflowTypeMetadata)Bindings[declaredType]).RuntimeType,
            source,
            CurrentPath,
            ...
            isDefault,
            CurrentModuleId);
        entry.AssignTargetIndex(_functions.Count);
    ...
    private string? CurrentValueType => _currentValues.Count == 0 ? null : _currentValues.Peek().DeclaredType;
    ...
    public T Resolve<T>(string declaredType, string key)
    {
        ...
        // 记录必须先统一建壳并注册，再填充字段；因此环上的每条引用都只查表，不递归构造对象。
        throw new InvalidOperationException(
            $"Record `{node.DeclaredType}::{key}` was referenced before snapshot shell registration.");
    }
    ...
    private long CurrentModuleId
    {
        get
        {
            if (_currentRecords.Count == 0) return 0;
            var current = _currentRecords.Peek();
            var record = FindRecord(current.DeclaredType, current.Key);
            return record is not null && _recordModuleIds.TryGetValue(record, out var moduleId)
                ? moduleId
                : 0;
        }
    }
    ...
    private string FormatReference(CfdFormatReference reference, CfdValueNode source)
    {
        ...
        var firstField = currentBinding.Find(reference.Path[0]);
        var objectType = firstField?.ObjectType;
        var referenceType = firstField?.ReferenceType;
        foreach (var field in reference.Path.Skip(1))
        {
            ...
            var metadataField = currentBinding.Find(field);
            objectType = metadataField?.ObjectType;
            referenceType = metadataField?.ReferenceType;
        }
        return Stringify(current);
    }
...
public interface ICfdTypeBinding
{
    ...
    IReadOnlyList<CoflowFieldMetadata> Fields { get; }
}
...
public static class CfdValueReader
{
    ...
    public static Option<T> Option<T>(CfdValueNode node, CfdLoadContext context,
        Func<CfdValueNode, CfdLoadContext, T> read) => node switch
        {
            ...
            CfdNoneValue => global::Coflow.Runtime.Option<T>.None,
            CfdSomeValue some => global::Coflow.Runtime.Option<T>.Some(read(some.Value, context)),
            _ => global::Coflow.Runtime.Option<T>.Some(read(node, context)),
        };
    ...
    public static Result<T, TError> Result<T, TError>(
        ...
        Func<CfdValueNode, CfdLoadContext, TError> readError) => node switch
        {
            ...
            CfdOkValue ok => global::Coflow.Runtime.Result<T, TError>.Ok(readValue(ok.Value, context)),
            CfdErrValue error => global::Coflow.Runtime.Result<T, TError>.Err(readError(error.Value, context)),
    ...
    public static void ValidateFields(IReadOnlyList<CfdFieldNode> fields, params string[] expected)
    {
        ...
        if (unknown is not null)
        {
            throw new CfdLoadException(new[]
            {
                new CfdDiagnostic(
                    "CFD-FIELD-UNKNOWN",
                    $"CFD field `{unknown.Name}` is not declared by the schema type",
                    string.Empty,
                    unknown.Span),
            });
        }
