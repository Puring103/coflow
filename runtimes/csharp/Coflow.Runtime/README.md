...
使用 `new Coflow.RuntimeBuilder(Generated.Contract)` 创建构建器，
`AddSource(text, sourceName: "可选诊断标签")` 提交 CFD 文本，`Build()` 获得只读运行时。
通过 `runtime.Table<Item>()["key"]` 和 `runtime.Singleton<Settings>()` 读取数据。
只释放 builder 与 Runtime，对象、集合、函数和 struct 包装无需单独 Dispose。
