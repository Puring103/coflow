...
namespace Coflow
{
    ...
    internal sealed class ProjectImage
    {
        ...
        internal bool NeedsLease(ProjectImage fixedSnapshot)
        {
        ...
        internal void Materialize(Runtime owner)
        {
            foreach (var pair in nodes) {
                ...
                if (!ReferenceEquals(this, owner.Image) && owner.Image.Contains(pair.Key))
                    pair.Value.Projection = owner.Image.Get(pair.Key).Projection;
                else pair.Value.Projection = owner.Contract.Binding(pair.Value.Type).Materialize(new Projection(owner, pair.Key, this));
            }
            IsMaterializing = true;
            try {
                foreach (var pair in nodes) {
                    if (pair.Value.Kind != 6 || (!ReferenceEquals(this, owner.Image) && owner.Image.Contains(pair.Key))) continue;
        ...
        internal static ProjectImage Read(byte[] bytes)
        {
            try
            {
