...
- Git Diff 源码支持 CFT/CFD 高亮，包括已删除文件；表格增加“仅修改列”筛选。
- Git 变更列表沿用项目文件树分组与目录层级，无语义记录变化时隐藏记录和表格视图。
- 基础记录删除后，残留维度记录静默忽略，并在构建时清理，保留有效记录的翻译。
- 维度校验在变体缺失或显式为 None 时回退到基础字段值，修复缺少翻译导致构建失败的问题。
...
- Git 差异读取改用 gix，修复 Windows 编辑器刷新变更时反复弹出终端的问题。
- 统一项目路径处理，修复绝对路径、父级目录、UNC 路径及外部维度目录的解析和重复展示。
- Git Diff 正确比较仓库内的外部项目源文件；仓库外文件排除并显示诊断。
- 生成内容比较忽略 CRLF/LF 和文件末尾换行数量差异，保留等价文件原始内容，避免无意义的构建提示与 Git 变更。
...
- 维度 CFD 统一使用 `__coflow_<维度名>_<类型名>_<字段名>` 辅助记录类型，修复 Rust 生成文件与 C# Runtime 加载协议不一致的问题。
- 修复 singleton 多字段维度写入，支持继承字段、多维度和默认值回退，并校验名称冲突及无效目标记录。
- 同步示例、使用文档与技能参考。现有项目需要更新旧格式维度文件并重新生成 C# 代码。
...
- 修复 C# 命名空间生成测试依赖 LF 换行符、在 Windows CI 的 CRLF 环境中失败的问题。
...
- 恢复 C# 代码生成的 `namespace` 配置，生成类型、Schema 绑定和维度类型使用配置的命名空间。
- 命名空间配置同步应用于生成代码的类型引用，并校验命名空间标识符；未配置或为空字符串时使用全局命名空间。
- 同步 C# 使用文档和 Coflow skills 引用。
...
- 项目现在会统一加载、展示和追踪 `coflow.yaml` 声明的外部 CFT/CFD 路径，包括 `../` 相对路径和绝对路径。
- CFD 编辑器会在配置变更后同步更新外部源目录监听，LSP 也会处理已声明项目路径中的文件变更。
- Coflow skills 已同步当前 CFD、CFT、项目流水线和 C# Runtime 使用契约。
...
- CFD 成为项目数据的唯一来源，CLI、编辑器、LSP、维度配置和生成流程使用一致的语法与模型。
- 新增 CFT / CFD 格式化命令，并完善默认值、函数、复合类型、字段引用和项目级检查的语言支持。
- 编辑器可以直接创建、修改和修复项目文件，对尚未通过校验的 CFD 内容也能继续编辑。

### 编辑器与扩展能力

- 新增基于语言服务器的 CFT / CFD 源码编辑、补全和诊断体验。
- 新增原生 Git 差异视图，便于在编辑过程中检查配置变更。
- 完善前端插件体系、检查器值树、表格复杂值展示和项目文件管理。

### C# Runtime

- C# Runtime 现在直接加载 CFD 源文件，并支持模块组合、函数值、闭包、Host 调用和维度变体。
- 生成代码与 Runtime 使用统一的元数据契约和寄存器执行模型，覆盖校验、资源限制及复杂值处理。
- Runtime 支持 `net8.0` 与 `netstandard2.1`，并提供完整的集成示例。
