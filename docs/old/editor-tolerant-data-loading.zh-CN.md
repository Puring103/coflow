...
编辑器为无法由默认值补全的必填字段生成 `missing = true` 单元格：能够自然生成初始值的字段使用类型默认值；必填引用保留带目标类型的缺失状态。schema 默认值和省略的 `Option<T>` 由核心模型正常物化，均不是缺失错误。禁止把其他类型错误伪装成缺失值，禁止从重复 key 中选择任意一条记录作为有效记录。被拒绝内容不得进入运行时索引、check、代码生成或发布模型。

表格、记录视图和检查器必须将 `missing = true` 渲染为统一的 `Missing` 状态。必填引用直接显示引用选择器，由用户明确选择目标；其他具有类型默认值的字段可以一次性提交该默认值。没有合法类型默认值时保持缺失，不写入伪造值。
