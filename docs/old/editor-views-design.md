...
- 普通字段编辑原位更新卡片；关系变化更新节点和连线，已有节点保持坐标，自动布局产生的新节点放到空闲区域。
- 左下图视图工具栏只有三个按钮：自动布局（布局图标，重新计算当前图并覆盖保存的位置）、回正（居中显示整张图）、缩略/完整模式切换。
- 一次节点拖动或手动重新布局作为一条编辑器历史记录，统一支持撤销和重做；初始布局和新增节点安置不新增历史步骤。
- 坐标保存不推进配置数据版本，不触发项目重载。缩略模式保留完整卡片尺寸和列表项端口位置。
- 缩略/完整模式由用户手动切换并按图视图持久化，不再随缩放自动切换；缺省为缩略模式。
- 图节点顶部显示 ID/缩略名，底部小字显示类型，不再与 Inspector 头部保持一致的字段布局。
- 缩略模式下的名称只显示缩略名或 ID（不显示“缩略名(ID)”），从较大字号开始按节点可用空间自适应缩小，长名称不被裁切。
- 自动布局以交叉最小化为目标（中线层扫描 + 网络单纯形放置 + 提高搜索强度），优先减少中段交叉。
- 右键拖动时显示真实框选范围，松开后收起为选中节点的包围盒：节点按与选框的重叠面积计入，连线按曲线采样与选框相交计入；框选拖动结束后即使落点在节点上也不弹出节点右键菜单。
- 节点拖动时只与屏幕内节点的左/中/右、上/中/下（含中心线对中心线）对齐吸附并显示对齐参考线，判定沿用 React Flow 官方 Helper Lines 思路；整组拖动按包围盒吸附，松手保持吸附结果不抖动。
- 连线从端口水平伸出一段、斜直线连接、水平进入目标端口，共三段；两端水平段始终朝节点外侧延伸，两处折点随内角减小逐渐过渡为圆角，钝角有轻微圆角、内角越尖锐曲度越大，避免尖点。连线与端口使用加粗高对比样式，便于识别和命中。
- 拖线到空白处时，目标下拉列表首项为“新建节点”；通过记录创建对话框建立记录后写入引用，并把新节点放在落点位置；创建记录与建立引用合并为一步撤销。
- 记录右键菜单在表格、记录、图三种视图中共用，按各自能力显示跳转、分组、重命名、下方插入和删除等项。
