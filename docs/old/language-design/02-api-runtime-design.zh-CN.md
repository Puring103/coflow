...
Rust schema 使用 `CftStaticValue` 表示已解析的常量和字段默认值；声明归属分别由常量声明和字段持有。
数组、字典和对象保留明确的类型结构，空集合使用对应集合变体。字段级依赖分析验证显式默认值，
必填对象的补齐使用一份类型依赖计划。构建期草稿与执行期值分别承担诊断预算和运行时身份管理。
Runtime 构建器、固定值 Arena 和映像组装集中在 `runtime/build.rs`；执行侧的对象初始化和局部构造
共用字段默认值规则。
