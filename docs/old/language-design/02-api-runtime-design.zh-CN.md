...
维度包装类型使用同一套 struct codec：默认值按原类型布局内联，随后存放一个变体字典集合 ID。
字段偏移和所属类型宽度均按包装后的布局计算；导入同时处理默认值与变体字典。
Schema 注册维度辅助记录名称，加载器保留这些记录供生成 reader 查询，普通表与单例仅发布主体记录。
辅助记录采用 `__coflow_<维度名>_<声明类型名>_<字段名>`，命名由 coflow-language 统一提供。
Schema 编译检查辅助名称冲突并保留 `__coflow_` 前缀；生成绑定注册完整名称到主体类型、字段和 singleton 标记的映射，加载器不拆分下划线推断归属。
普通记录沿用主体 key，singleton 以字段名作为辅助 key，同一文件中的每行保留各自的辅助类型。
C# 在构建候选快照时验证辅助记录对应的主体存在，生成 reader 读取合法变体并构造维度包装值。
