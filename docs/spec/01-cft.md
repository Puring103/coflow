...
对象字段的数据可以是内联对象，也可以是数据源中的显式记录引用。Excel 单元格用 `@key` 引用记录，用 `@key.path[index]` 引用某条记录的字段或集合元素。引用解析在 DataModel build 阶段执行，并按字段的声明类型检查兼容性：子类记录可以赋给父类字段，父类记录不能赋给子类字段。
