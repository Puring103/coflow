...
如果 `ExcelSource.sheets` 为空，加载器默认加载 workbook 中全部 sheet：sheet 名作为 CFT 类型名，表头文本作为字段名。需要重命名 sheet、覆盖类型名、覆盖 key 列或映射显示列头时，上层传入显式 `sheets` 配置。`key` 省略时默认使用 `id` 表头列；`columns` 是列头重命名映射，不限制未列出的字段。
...
对每个数据行，先读取 key 列作为 record key；默认 key 列名为 `id`，也可由 `key` 配置覆盖。key 列不映射到 CFT 字段，且 CFT 禁止声明名为 `id` 的字段。随后按列名（经过 `columns` 映射后）找到对应字段，根据字段的 CFT 类型 schema-guided 解析单元格内容（见第 3 节）。
