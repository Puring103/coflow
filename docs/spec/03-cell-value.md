...
- `Value`：结构化输入值，后续交给数据模型做字段必填、默认值、重复 key、记录引用解析和类型兼容校验
...
目标类型为 `bool` 时，单元格还接受以下大小写不敏感的别名：`1`、`yes`、`y` 视为 `true`；`0`、`no`、`n` 视为 `false`。这种放宽只发生在目标类型为 `bool` 的位置——目标类型是 `string` 时，`1` / `yes` 等仍是字符串内容。
...
对象支持按位置填写和按字段名填写两种风格。一个对象内不能混用这两种风格。
...
按字段名填写时，`field: _` 等价于不写该字段，但通常直接省略字段更清楚。
...
```
// Monster { level: int; stats: Stats; }，record key 由外层记录的 id 列提供
slime, 5, {100, 50}
