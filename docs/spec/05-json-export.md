...
JSON exporter 位于 `coflow-exporter-json`，实现 `coflow-api::DataExporter`，并与 MessagePack exporter 共用 `coflow-exporter-core` 的 schema-aware 遍历规则。

---

## 文件结构

JSON 导出产物是一个输出目录。每个有记录的非 `abstract` CFT type 都导出为一个
`<TypeName>.json` 文件；没有记录的 table 不写空 `[]` 文件。文件内容是该 table 的记录数组，保持数据源顺序：
...
`CfdDataModel` 内部的 enum value 会携带 enum 类型名；JSON 不重复写出类型名，只导出底层整数值，由字段 schema 决定 enum 类型。
...
导出前的 `CfdDataModel` 已保证字典 key 唯一。JSON 文本中如果出现重复 object key，运行时加载器必须按格式错误处理，不允许依赖 JSON parser 的后写覆盖行为。
...
"rewards": [
  ...
  { "$type": "ItemReward", "id": "r2", "item": "sword_01", "count": 1 }
]
```
...
引用字段导出为目标 record key，不内联目标对象，避免数据膨胀。JSON 格式本身可以表达循环引用：
...
运行时加载器根据字段 schema 的目标类型和 key 索引解析引用。C# 只读即时解析加载器要求被引用 table 先加载；如果 table 之间存在循环引用，C# codegen 会拒绝生成。
...
[
  {
    ...
    "drops": {
      "rewards": [
        ...
        { "$type": "ItemReward", "id": "r2", "item": "potion_01", "count": 1 }
      ],
      "weights": [60, 40]
    },
    "boss_drop": null,
