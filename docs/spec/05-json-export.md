...
JSON 导出产物是一个输出目录。每个非 `abstract` CFT type 都导出为一个
`<TypeName>.json` 文件；即使该 table 没有记录，也会写出空数组。文件内容是该 table 的记录数组，保持数据源顺序：
...
引用字段导出为目标 record key，不内联目标对象，避免数据膨胀并允许循环引用：
...
运行时加载器根据字段 schema 的目标类型和 key 索引解析引用。允许循环引用（加载器两遍设计天然支持）。
