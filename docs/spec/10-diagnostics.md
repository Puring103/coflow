...
Coflow 诊断是项目配置校验、CFT 编译、Excel 加载、data model 构建、引用解析、
...
Excel cell 已解析后的 data model 构建诊断。
...
| 诊断码 | 含义 |
| --- | --- |
| `CFD-REF-001` | 引用目标类型没有 ID |
| `CFD-REF-002` | 找不到被引用的目标记录 |
...
- `build`：存在 project、schema、Excel、data model、reference、check、codegen
...
3. 修 `CFT-*`；schema 错误会阻塞 Excel 和 artifact 阶段。
4. 修 `EXCEL-*` 和 `CELL-*`；这些错误会阻塞 data model 构建。
