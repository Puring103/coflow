...
Coflow 诊断是项目配置校验、CFT 编译、loader resolve/preflight/load、data model
构建、引用解析、`check {}` 执行、codegen preflight、artifact preflight/staging/commit
产生的结构化错误。
...
- source `path` 为空或不存在。
- source `url` 为空。
- source 同时设置 `path` 和 `url`，或二者都没有设置。
- output type 不支持。
- output directory 为空。
...
- `@expand` 后续被消费的相邻列存在非空表头。被消费列必须紧邻父列且表头为空，
  或由 Excel 合并单元格让非左上角表头单元格为空。
...
| 诊断码 | 含义 |
| --- | --- |
| `CFD-CHECK-001` | 兜底 false 条件，只有无法归类到更具体规则时使用 |
| `CFD-CHECK-002` | check 求值运行期类型错误 |
| `CFD-CHECK-003` | 访问 null |
| `CFD-CHECK-004` | 数组索引越界 |
| `CFD-CHECK-005` | 字典 key 不存在 |
| `CFD-CHECK-006` | `min` / `max` 没有非 null 值 |
| `CFD-CHECK-007` | 比较条件失败 |
| `CFD-CHECK-008` | 裸 bool 表达式期望为 true 但实际为 false |
| `CFD-CHECK-009` | `!expr` 失败，内部表达式实际为 true |
| `CFD-CHECK-010` | `lhs && rhs` 失败 |
| `CFD-CHECK-011` | `lhs || rhs` 失败 |
| `CFD-CHECK-012` | `expr is TypeName` 类型谓词失败 |
| `CFD-CHECK-013` | null 谓词失败，包括 `is null`、`== null`、`!= null` |
| `CFD-CHECK-014` | `contains` 返回 false |
| `CFD-CHECK-015` | `unique` 返回 false |
| `CFD-CHECK-016` | `matches` 返回 false |
| `CFD-CHECK-017` | `any` 量词没有任何元素满足 |
| `CFD-CHECK-018` | `none` 量词存在满足条件的元素 |
| `CFD-CHECK-019` | `all` 量词存在不满足条件的元素 |
...
`when` 不分配独立错误码。`when` 条件为 true 且 body 内规则失败时，诊断使用
...
artifact 诊断用于输出目录安全检查、artifact path 检查、staging、commit 和
lockfile 读写/解析失败。

示例：

- 输出路径已经存在且不是目录。
- 输出目录指向项目根、schema 目录或 source 目录。
- data/code 输出目录互相重叠。
- provider 返回空路径、绝对路径或会逃逸输出目录的 artifact path。
- staging 目录创建失败或 artifact 文件写入失败。
- enum lockfile 读取、解析、staging 或提交失败。
...
| Artifact staging/commit | 否 | 单次文件系统失败会停止当前写入阶段 |
...
preflight 后发生的真实 filesystem 写入失败报告为 `ARTIFACT-001`，并使用
`SourceLocation::Artifact` 定位到相关输出目录、staging 文件或 lockfile。

数据导出和 C# codegen 会先写入同级 staging 目录，全部文件写入成功后再替换
目标输出目录。C# codegen 的 `coflow.enum.lock.json` 位于 `coflow.yaml` 同级，
并通过 staging 文件提交；如果目录提交失败，Coflow 会尽力回滚已替换的 lockfile。
