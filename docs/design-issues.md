...
CFT 已有错误码触发覆盖测试，`CfdErrorCode` 已建立“负向触发 + 相邻合法输入不误报”
机械覆盖，`CFD-TEXT-*` 已覆盖负向和相邻合法输入。但同一标准还没有统一覆盖到所有错误码体系。
...
- 为 CFT、cell value、Excel loader、pipeline/artifact/codegen 逐步建立错误码覆盖清单。
