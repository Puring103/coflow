...
## Source 扩展名大小写敏感（设计约束）

pipeline 按扩展名字符串精确识别 `.xlsx`、`.xlsm`、`.xls` 和 `.cfd`。大小写不同的
`.XLSX` 或 `.CFD` 在目录扫描时会被忽略，在显式文件源中会被判定为不支持。

影响：

- 文件命名规范需要明确要求精确小写扩展名。
- 目录扫描和显式文件源要保持同一套大小写敏感规则，避免本地工具和 CLI 表现漂移。

建议方向：

- 已补充目录源 `.XLSX` / `.CFD` 被忽略，以及显式 `.CFD` 文件源报 unsupported extension 的测试。
- 文档中避免把大写扩展名描述成可接受输入。
