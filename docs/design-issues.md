...
`coflow-checker` 的部分内建函数使用首个参数执行，例如 `len`、`unique`、`min`、
`max`、`sum`、`keys`、`values` 等。额外参数可能被静默忽略，`contains` 对非集合
值也可能返回 `false` 而不是类型错误。

影响：

- 错误的 check 规则可能被当作普通布尔失败或在 `when` 中被跳过。
- `CFD-CHECK-EVAL-TYPE` 的覆盖不足，无法稳定发现这类规则错误。

建议方向：

- 为每个内建函数声明参数个数和参数类型，运行时严格校验。
- 补充负向测试：`len(items, 1)`、`sum(values, 1)`、`keys(dict, 1)`、`contains(123, 1)`。
- 补充相邻合法测试：单参数集合函数、合法 `contains(array, value)`、合法 `contains(dict, key)`。
...
Excel loader 对 `@expand` 字段按内层字段数量位置消费后续列，并忽略这些被消费列的
表头文本。
...
- 对被 `@expand` 消费的后续 header 做约束：必须为空、显式子字段名，或采用清晰的
多列表头约定。
- 为 `@expand` 后接普通列、子列缺失和子列解析错误补充测试。
- 在 origin 中记录 expand 子字段到具体 Excel 列的映射。

## Source 扩展名大小写敏感

pipeline 目前按扩展名字符串识别 `.xlsx`、`.xlsm`、`.xls` 和 `.cfd`。大小写不同的
...
- source kind 判断改为 ASCII lowercase 后匹配。
- 补充显式文件源和目录源的大小写扩展名测试。
