...
- `CheckDependencyPlan` 完成后替换并删除旧的 `dimension_checks` 近似 walker；不保留两套 statement-to-dimension 计算。`TypedCheckPlan` 只负责 schema 调度索引和查询，不继续隐藏另一套表达式语义分析。
...
8. 将 dimension AST walker 重构为带静态类型的通用 `CheckDependencyPlan`，先覆盖现有 type-local check，用回归测试证明行为一致后删除 `dimension_checks` 旧 walker。
...
| `CheckDependencyPlan` | typed check query、dimension schedule、top-level/type-local checks | `dimension_checks` walker 及其他 statement-to-dimension 近似遍历 |
