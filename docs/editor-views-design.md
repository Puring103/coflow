...
- 右键拖动时显示真实框选范围，松开后收起为选中节点的包围盒：节点按与选框的重叠面积计入，连线按曲线采样与选框相交计入。
- 包围盒作为整组拖动手柄，拖动它连同内部节点一起移动，点击别处取消选择并隐藏。
- 连线支持从起点、终点和线体重连：线体按落点选择较近的一端；拖到空白处删除原关系。连线与端口使用加粗高对比样式，便于识别和命中。
