...
- `Option<T>` 字段没有默认值时仍必须填写；如果希望 CFD 可省略，写 `= None`。
- 普通字符串默认值可以通过 `{field}` 插值引用当前对象字段；业务约束仍应写在 `check {}` 里。
...
- 读取 `Option<T>` 内部值前，先用 `is Some(value)` 模式绑定；可在 `when` 条件或短路表达式中使用绑定值。
...
- 把所有东西都内联。需要跨 CFD 文件复用、查找或保持唯一身份时，用顶层记录和 `&Type`。
