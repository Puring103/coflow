---
name: coflow-data
description: "Coflow 数据文件与数据源维护：当用户需要编写或修改 .cfd、Excel/CSV/飞书/Lark 表格数据、创建数据文件、同步表头、添加/修改/删除/重命名记录、使用 data patch、处理单元格值语法或修复 schema/data 不一致时使用。"
---
...
4. 少量记录增删改用 `coflow data patch <project> --patch patch.json`。
5. 复杂 CFD 整文件整理用 `coflow data write-file <project> --file data/file.cfd --stdin --check`。
6. 表格和远端 source 不用整文件写入；通过 provider writer 和 `data patch` 修改。
...
- Excel/CSV/Lark 单元格值语法：读 `references/cell-value.md`，公开链接 <https://puring103.github.io/coflow/docs/reference/03-language/03-cell-value>。
...
- 表格、Excel、CSV、飞书/Lark 细节：按需读取 `references/table-source.md`、`references/excel.md`、`references/csv.md`、`references/lark.md`。
