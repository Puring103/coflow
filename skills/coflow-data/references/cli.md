...
执行只依赖 schema 的 `codegen csharp`；添加实际类型和记录后，`build`/`export` 才会产生有
业务内容的数据表。
...
| `--namespace NAME` | 覆盖第一个 `code.type: csharp` target 的命名空间 |
...
`outputs.data.dir` 或 `--out` 是导出成功后消费者直接读取的目录。命令成功信息输出该目录。
...
C# 代码会写入所选 target 的 `code.dir` 或 `--out` 指定的目录，命令成功信息输出该目录。
生成失败时旧目录保持不变。

对于 `@idAsEnum`，单独运行 `codegen csharp` 会复用已有编号；干净 clone 可以从应提交到版本库的
