...
单元格值语法用于在 Excel 和 CSV 单元格中表示复杂类型。它是 schema-guided 的：每个单元格都会按 CFT 字段类型解析。
...
string 单元格可以使用和 CFD 相同的字段引用语法，不需要额外前缀：

```text
{name}
获得 <b>{&sword.name}</b>
售价 {&Item::sword.price} 金币
```

- `{field}` 引用当前记录的字段。
- `{&key.field}` 引用当前类型另一条记录的字段。
- `{&Type::key.field}` 显式指定目标类型、记录 key 和字段。

字段路径可以继续穿过内联对象或记录引用。字面量 `{`、`}` 分别写成 `{{`、`}}`。项目加载完成后才解析引用；循环引用、目标记录或字段不存在、路径无法继续访问都会产生诊断。check、导出和代码生成使用求值后的普通 string，表格 writer 则保留格式化字符串源码。
