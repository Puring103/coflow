...
Coflow 通过 Provider 读取和写回本地数据源。CFT 负责定义结构，Provider 负责把 Excel、CSV 和 CFD 来源转换成统一的输入记录，再交给 DataModel 做默认值、引用解析和业务校验。
...
本地目录 source 会递归发现支持的文件。单文件 source 可以省略 `type`，由 Provider registry 探测；如果多个 Provider 同等匹配，应显式设置 `type`。
...
| 问题 | 原因 | 处理 |
| --- | --- | --- |
| source 缺少 `path` | source 入口不明确 | 指定本地文件或目录 |
| 目录里文件未加载 | 没有 Provider 支持该扩展名 | 使用 `.xlsx`、`.csv`、`.cfd` |
