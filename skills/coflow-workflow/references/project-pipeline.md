...
项目字段和路径规则见 [`coflow.yaml` 参考](./project-config.md)。CLI 参数以及各命令使用的
流水线阶段见 [CLI 命令参考](./cli.md#命令矩阵)。
...
以及批量修改记录。具体操作和参数见 [CLI 数据命令](./cli.md#data)。
...
输出目录内的 `.meta` 文件会在替换输出目录时原样保留，Coflow 不判断它们是否仍有对应
产物；这些文件的创建、清理和 GUID 管理由 Unity 负责。输出目录自身的 `.meta` 文件位于
目录外，不受替换影响。
