...
引用可跨 CFD 文件，也允许自引用和跨记录循环。目标必须存在且实际 type 可赋给声明的引用类型。引用不是
字符串，不能使用引号。
...
插值引用形式为 `{field}`、`{&key.field}` 或 `{&Type::key.field}`。<code>&#123;&#123;</code> 和 <code>&#125;&#125;</code> 表示字面花括号。
...
数据模型。多个来源按稳定路径顺序加载。维度输出使用
`__coflow_<维度名>_<类型名>_<字段名>` 记录类型，明确标识目标字段；文件组织见[本地化与维度](https://puring103.github.io/coflow/docs/reference/10-localization)。
