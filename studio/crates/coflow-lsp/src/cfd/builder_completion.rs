...
fn expression_type(expr: &Expr, schema: &CftSchema, scopes: &[Scope]) -> Option<CftValueType> {
    match &expr.kind {
        ...
        ExprKind::Field { value, name } => {
            let CftValueType::Object(ty) = expression_type(value, schema, scopes)? else {
                return None;
            };
            Some(schema.field(&ty, name)?.value_type.clone())
        }
        ExprKind::Index { value, .. } => match expression_type(value, schema, scopes)? {
            CftValueType::Array(ty) | CftValueType::Dict(_, ty) => Some(*ty),
            _ => None,
        },
        ExprKind::Object { type_name, .. } => Some(CftValueType::Object(
            schema.resolve_type(type_name)?.name.clone(),
        )),
        _ => None,
    }
}
    ...
    if let Ok(function) = parse_function(&format!("{signature} {{}}")) {
        for (name, ty) in function.parameters {
            scopes[0].insert(
                name,
                Local {
                    ty: schema.resolve_type_ref(&ty).ok(),
                    builder: false,
                },
            );
        }
    ...
    for index in 0..dot {
        match text(index) {
            "var" => {
                ...
                let ty = if text(index + 2) == ":" {
                    (index + 3..dot)
                        ...
                        .and_then(|equal| {
                            ...
                            let function = parse_function(&format!(
                                "fn(value: {}) -> int {{}}",
                                &prefix[start..end]
                            ))
                            .ok()?;
                ...
                scopes
                    .last_mut()?
                    .insert(name.into(), Local { ty, builder: false });
            }
            "build" => {
                // 整个头部交回正式解析器，避免另造一份集合/命名类型文法。
                let Some(as_index) = (index + 1..dot).find(|i| text(*i) == "as") else {
                    continue;
                };
                if text(as_index + 2) != "{" {
                    continue;
                ...
                let Ok(expr) = parse_expression(&format!("{head} {{}}")) else {
                    continue;
                };
                let ExprKind::Build {
                    source, binding, ..
                ...
                let ty = match source {
                    BuildSource::Type(ty) => schema.resolve_type_ref(&ty).ok(),
                    BuildSource::Value(value) => expression_type(&value, schema, &scopes),
                };
                pending.insert(as_index + 2, (binding, Local { ty, builder: true }));
            }
            "{" => {
                let mut scope = Scope::new();
                if let Some((name, local)) = pending.remove(&index) {
    ...
    match local.ty.as_ref()? {
        CftValueType::Object(name) => {
            for field in schema.resolve_type(name)?.all_fields() {
                items.push(LanguageCompletion {
                    ...
                    kind: Some(5),
                    detail: Some((field.value_type.display_label()).to_string()),
                    ..Default::default()
                });
            }
        }
        CftValueType::Array(inner) => {
            items.push(LanguageCompletion {
                ...
                kind: Some(2),
                detail: Some((format!("append(value: {})", inner.display_label())).to_string()),
                insert_text: Some(("append(${1:value})").to_string()),
            ...
            items.push(LanguageCompletion {
                ...
                kind: Some(2),
                detail: Some(("remove(index: int)").to_string()),
                insert_text: Some(("remove(${1:index})").to_string()),
                insert_text_format: Some(2),
                ..Default::default()
            });
        }
        CftValueType::Dict(key, _) => items.push(LanguageCompletion {
            label: ("remove").to_string(),
            kind: Some(2),
            detail: Some((format!("remove(key: {})", key.display_label())).to_string()),
...
mod tests {
    ...
    fn typed_builder_members_follow_source_types_and_scope() {
        ...
        let schema = build_schema(&parse_modules([CftFile::from_source(
            ...
            "data Item { count: int; title: string; }",
        )]))
        .unwrap();
        let labels = |signature: &str, prefix: &str| {
            members(signature, prefix, &schema)
                .map(|items| items.into_iter().map(|item| item.label).collect::<Vec<_>>())
        };
        assert_eq!(
            labels("fn() -> [int]", "fn() -> [int] { build [int] as b { b."),
            Some(vec!["append".into(), "remove".into()])
        );
        assert_eq!(
            labels(
                "fn() -> {string: int}",
                "fn() -> {string: int} { build {string: int} as b { b.re"
            ),
            Some(vec!["remove".into()])
        );
        assert_eq!(
            labels("fn() -> Item", "fn() -> Item { build Item as b { b."),
            Some(vec!["count".into(), "title".into()])
        );
        assert_eq!(
            labels(
                "fn(input: [int]) -> [int]",
                "fn(input: [int]) -> [int] { build (input) as b { b."
            ),
            Some(vec!["append".into(), "remove".into()])
        );
        assert_eq!(
            labels(
                "fn() -> [int]",
                "fn() -> [int] { var original: [int] = []; build (original) as b { b."
            ),
            Some(vec!["append".into(), "remove".into()])
        );
        ...
        assert!(labels(
            "fn() -> int",
            "fn() -> int { build [int] as b { var b: int = 0; b."
        )
        .is_none());
    }
}
