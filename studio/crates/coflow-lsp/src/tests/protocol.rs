...
fn formatting_requests_handle_idempotent_unknown_and_dirty_documents() {
    ...
    assert!(edits.iter().any(|edit| {
        edit["range"]["start"] == json!({ "line": 1, "character": 0 }) && edit["newText"] == "  "
    }));
}
...
fn unified_diagnostic_severity_is_preserved_in_lsp_rendering() {
    for (severity, expected) in [
        ...
        let diagnostic = coflow_project::Diagnostic {
            code: "TEST".to_string(),
...
fn lsp_diagnostic_renders_structured_context_without_mutating_message() {
    let diagnostic = coflow_project::Diagnostic {
        ...
        severity: coflow_project::Severity::Error,
        message: "custom".to_string(),
        primary: None,
        related: Vec::new(),
        contexts: vec![coflow_project::DiagnosticContext {
            kind: "when".to_string(),
            expression: Some("enabled".to_string()),
            ..coflow_project::DiagnosticContext::default()
        }],
    };
