...
fn uses_the_published_session_instead_of_rereading_working_files() {
    ...
    assert_eq!(coflow_project::value_summary(after), "2");
...
fn excludes_sources_outside_repository_with_diagnostics() {
    ...
    let config = serde_json::json!({"schema":"schema.cft", "data":["data/", coflow_project::path_to_slash(&external_file)], "codegen":[{"language":"csharp", "dir":"generated"}]});
    fs::write(
        repo.path().join("coflow.yaml"),
        serde_yaml::to_string(&config).expect("yaml"),
    )
