...
use coflow_project::SourceLocation;
use coflow_project::{load_cfd_model, parse_cfd_input_records, CfdTextErrorCode, CfdTextLoadError};
use coflow_project::{CfdValue, LoadedValueDraft};
use std::fs;
...
fn runtime_parity_fixture(name: &str) -> String {
    fs::read_to_string(
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../../engine/tests/fixtures/cfd-runtime-parity")
            .join(name),
    )
...
fn direct_model_errors_keep_record_text_spans() -> TestResult {
    ...
    let mapped = coflow_project::map_diagnostics_with_origins(diagnostics, &origins);
    let primary = mapped
        .diagnostics
        .first()
