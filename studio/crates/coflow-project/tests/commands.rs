...
fn write_id_as_enum_project(is_flag: bool, records: &str) -> TempDir {
    ...
    fs::write(
        dir.path().join("schema.cft"),
        format!("{flag}enum ItemId {{}}\n@idAsEnum(ItemId) table Item {{ name: string; }}\n"),
    )
    .expect("schema");
    fs::create_dir_all(dir.path().join("data")).expect("data dir");
...
fn id_as_enum_values_are_stable_flag_safe_and_rename_aware() {
    ...
    coflow_project::commands::generate_project_code(&project).expect("generate initial enum");
    ...
    coflow_project::commands::generate_project_code(&project).expect("regenerate stable enum");
    ...
    coflow_project::commands::generate_project_code(&flag_project).expect("generate flag enum");
...
fn codegen_rejects_outputs_that_overlap_project_inputs() {
    ...
    for output in [".", "data/generated", ".coflow/artifacts/nested"] {
        ...
        let error = coflow_project::commands::generate_project_code(&opened)
...
fn cli_codegen_dispatches_through_the_language_registry() {
    let project = write_project();
    let opened = Project::open(Some(&project.path().join("coflow.yaml"))).expect("open data");
    let outcome = coflow_project::commands::generate_project_code(&opened).expect("generate code");
    let coflow_project::commands::CommandOutcome::Success(report) = outcome else {
    ...
    assert!(project
        .path()
        .join("generated/csharp/coflow.contract")
        .is_file());
}
...
fn csharp_codegen_rejects_removed_numeric_width_options() {
    ...
    assert!(error.to_string().contains("unsupported C# option `int_32`"));
...
fn csharp_codegen_applies_namespace_option() {
    ...
    coflow_project::commands::generate_project_code(&opened).expect("generate namespaced C#");
...
fn codegen_failure_does_not_publish_an_earlier_target() {
    ...
    .expect("config");
    let opened = Project::open(Some(&project.path().join("coflow.yaml"))).expect("open data");
    assert!(coflow_project::commands::generate_project_code(&opened).is_err());
...
fn generated_status_ignores_line_endings_and_preserves_equivalent_files() {
    ...
    coflow_project::commands::generate_project_code(&project).expect("generate");
    ...
    for text in [
        ...
        coflow_project::commands::generate_project_code(&project).expect("repair other file");
        assert_eq!(
...
fn codegen_preserves_unity_meta_without_generation_history() {
    ...
    coflow_project::commands::generate_project_code(&project).expect("generate baseline");
    ...
    coflow_project::commands::generate_project_code(&project).expect("reuse unchanged output");

    fs::write(
        project_dir.path().join("schema.cft"),
        "table Item { name: string; level: int = 1; }\n",
    )
    .expect("change schema");
    let changed = Project::open(Some(&config)).expect("reopen changed project");
    coflow_project::commands::generate_project_code(&changed).expect("generate changed output");
    assert_eq!(
...
fn csharp_codegen_emits_dimension_metadata_without_source_paths() {
    ...
    fs::write(
        dir.path().join("schema.cft"),
        "table UiText { @localized welcome: string; }\n",
    )
    .expect("schema");
    fs::create_dir_all(dir.path().join("data")).expect("data dir");
    fs::write(
        dir.path().join("data/base.cfd"),
        "main: UiText { welcome: dimension { default: \"Hello\", zh: \"你好\" } }\n",
    )
    .expect("base CFD");
    fs::write(
        dir.path().join("coflow.yaml"),
    ...
    let outcome = coflow_project::commands::generate_project_code(&project).expect("generate C#");
    assert!(matches!(
        outcome,
        coflow_project::commands::CommandOutcome::Success(_)
    ));
...
#[test]
fn csharp_codegen_reads_each_singleton_dimension_field() {
    let dir = tempfile::tempdir().expect("singleton dimension project");
    fs::write(
        dir.path().join("schema.cft"),
        "singleton UiText { @localized welcome: string; @localized farewell: string; }\n",
    )
    .expect("schema");
    fs::create_dir_all(dir.path().join("data")).expect("data dir");
    fs::write(
        dir.path().join("data/base.cfd"),
        "UiText: UiText { welcome: dimension { default: \"Hello\", zh: \"你好\" }, farewell: dimension { default: \"Bye\", zh: \"再见\" } }\n",
    )
    .expect("base CFD");
    fs::write(
        dir.path().join("coflow.yaml"),
...
#[test]
fn runtime_rejects_plain_values_for_dimension_fields() {
    let dir = tempfile::tempdir().expect("dimension project");
    fs::write(
        dir.path().join("schema.cft"),
        "table UiText { @localized welcome: string; }\n",
    ...
    let diagnostic = diagnostics
        .diagnostics
        .iter()
        .find(|diagnostic| diagnostic.code == "CFD-TEXT-TypeMismatch")
        .expect("dimension syntax diagnostic");
    let primary = diagnostic
        .primary
        .as_ref()
        .expect("dimension type source span");
    let coflow_project::SourceLocation::FileSpan {
        path,
        start_line,
        start_character,
        end_line,
        ...
        panic!("dimension diagnostic should point into the business CFD");
    };
    let expected_path = coflow_project::canonicalize_path(dir.path().join("data/base.cfd"))
        .expect("canonical dimension CFD path");
    assert_eq!(path, &expected_path);
    assert_eq!((*start_line, *start_character), (0, 24));
    assert_eq!((*end_line, *end_character), (0, 31));
}

#[test]
fn runtime_accepts_variants_declared_only_in_business_data() {
    let dir = tempfile::tempdir().expect("dimension project");
    fs::write(
        dir.path().join("schema.cft"),
        "singleton UiText { @localized welcome: string; @localized farewell: string; }\n",
    ...
    fs::write(
        dir.path().join("data/base.cfd"),
        "UiText: UiText { welcome: dimension { default: \"Hello\", zh: \"你好\", mod: \"Modded\" }, farewell: dimension { default: \"Bye\", zh: \"再见\" } }\n",
    )
    .expect("base CFD");
    fs::write(
        dir.path().join("coflow.yaml"),
