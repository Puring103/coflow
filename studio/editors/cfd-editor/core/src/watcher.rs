...
fn watch_roots(project: &Project) -> Vec<PathBuf> {
    let project_root = coflow_project::normalize_path(project.root_dir());
    let mut roots = vec![project_root.clone()];
    for source_root in project.source_roots() {
        if !coflow_project::path_is_same_or_descendant(&source_root, &project_root)
            && source_root.exists()
        {
...
fn normalize_paths(paths: &[PathBuf]) -> Vec<String> {
    let mut out = paths
        .iter()
        .filter(|path| !is_ignored_path(path))
        .map(|path| coflow_project::path_to_slash(&coflow_project::normalize_path(path)))
        .collect::<Vec<_>>();
    out.sort();
    out.dedup();
    out
...
mod tests {
    ...
    fn configured_external_source_is_watched() {
        ...
        assert_eq!(
            watch_roots(&project),
            vec![
                coflow_project::normalize_path(&project_root),
                coflow_project::normalize_path(&external),
            ]
        );
