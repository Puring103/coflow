...
impl RevisionCoordinator {
    ...
    pub(super) fn commit_reload(&self, ticket: RevisionTicket) -> Option<Self> {
        (ticket.0 == self.current).then(|| Self {
            ...
            // A reload may be the final phase of a host-initiated source write.
            // Keep its fingerprints until the asynchronous watcher observes the
            // filesystem events generated by that write.
...
fn resolve_path(project_root: &Path, path: &Path) -> PathBuf {
    ...
    coflow_project::normalize_path(&path)
}
...
mod tests {
    ...
    #[test]
    fn reload_preserves_internal_write_fingerprints_for_the_watcher() {
        let root = std::env::temp_dir().join(format!(
            "coflow-editor-reload-fingerprint-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(&root).expect("create temp directory");
        ...
        coordinator.commit_internal_write(&root, [path.as_path()]);
        let ticket = coordinator.begin_reload();
        let reloaded = coordinator.commit_reload(ticket).expect("commit reload");

        assert!(!reloaded.has_external_change(&root, std::slice::from_ref(&path)));
        fs::remove_dir_all(root).expect("remove temp directory");
    }
