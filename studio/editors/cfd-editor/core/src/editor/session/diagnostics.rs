...
use coflow_project::RecordCoordinate;
...
fn normalize_target(
    ...
    source_range: Option<coflow_project::TextRange>,
                ...
                let range = source_range.or_else(|| {
                    queries
                        ...
                        .and_then(|record| match &record.origin {
                            coflow_project::RecordOrigin::File { span, .. } => *span,
                            coflow_project::RecordOrigin::None => None,
                        })
                        .map(|span| {
                            coflow_project::TextRange::from_parts(
                                span.start_line,
...
fn project_relative_path(project_root: &Path, path: &str) -> String {
    coflow_project::project_path(project_root, Path::new(path))
}
