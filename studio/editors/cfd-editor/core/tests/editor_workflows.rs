...
use coflow_project::{CfdObject, CfdPathSegment, CfdValue, RecordCoordinate};
...
fn configured_external_sources_can_be_opened() {
    ...
    assert_eq!(
        ...
        coflow_project::canonicalize_path(&data_path).expect("canonicalize external data")
    );
    let external_schema = coflow_project::path_to_slash(
        &schema_path
            .canonicalize()
    ...
    assert_eq!(
        ...
        coflow_project::canonicalize_path(&schema_path).expect("canonicalize external schema")
    );
...
fn repository_projects_open_in_editor() {
    ...
    for (project, config) in [
        ...
        assert_eq!(
            ...
            coflow_project::path_to_slash(&expected_root)
        );
        assert!(
            snapshot
                .diagnostics
    ...
    let outcome = store
        .edit_collection(
            ...
            CollectionEdit::DictInsert {
                key: coflow_project::CfdDictKey::Enum(coflow_project::CfdEnumValue {
...
fn deep_default_parents_materialize_for_collection_and_batch_edits() {
    ...
    let enum_key = coflow_project::CfdDictKey::Enum(coflow_project::CfdEnumValue {
        enum_name: "Element".try_into().expect("enum name"),
...
fn source_text_edit_saves_invalid_data_with_complete_diagnostics() {
    ...
    assert!(
        diagnostics.iter().any(|diagnostic| matches!(
            &diagnostic.target,
            coflow_project::DiagnosticTarget::Source { range: Some(range), .. }
    ...
    assert!(bootstrap
        ...
        .any(|diagnostic| diagnostic.severity == "error"
            && matches!(
                &diagnostic.target,
                coflow_project::DiagnosticTarget::Source { .. }
            )));
    assert_eq!(
...
fn missing_optional_default_ref_and_enum_states_are_structurally_repairable() {
    ...
    assert!(snapshot.diagnostics.iter().any(|diagnostic| {
        diagnostic.code == "DATA-006"
            && matches!(&diagnostic.target, coflow_project::DiagnosticTarget::TableField { field_path, .. } if field_path == "name")
    }));
    assert!(snapshot.diagnostics.iter().any(|diagnostic| {
        diagnostic.code == "DATA-008"
            && matches!(&diagnostic.target, coflow_project::DiagnosticTarget::TableField { field_path, .. } if field_path == "rarity")
    }));
    ...
    let repaired = store
        .write_field(
            ...
            &[field("rarity")],
            &CfdValue::Enum(
                coflow_project::CfdEnumValue::try_new("Rarity", Some("Rare"), 1)
                    .expect("enum value"),
            ),
        )
...
fn new_record_can_be_created_with_a_missing_required_reference() {
    ...
    assert!(outcome.diagnostics.iter().any(|diagnostic| {
        diagnostic.severity == "error"
            && matches!(&diagnostic.target, coflow_project::DiagnosticTarget::TableField { field_path, .. } if field_path == "target")
    }));
