use crate::ast::{
    BinOp, CheckBlock, CheckExpr, CheckExprKind, CmpOp, CondStmt, DataDef, Expr, ExprKind,
    ModuleAst, ObjectField, PathSegment, QuantifierKind, TypeName, TypePredicate, TypeRef, UnaryOp,
};
...
impl Parser {
    ...
    fn parse_module(&mut self) -> Result<ModuleAst, ParseErrors> {
        ...
        while !self.at(&TokenKind::Eof) {
            if self.at(&TokenKind::Use) {
                return self.err("use declarations are not valid in .cfd files");
            }
            if self.at(&TokenKind::Type) {
                return self.err("type definitions are not valid in .cfd files");
            }
            if self.at(&TokenKind::Enum) {
                return self.err("enum definitions are not valid in .cfd files");
            }
            if self.at(&TokenKind::Check) {
                checks.push(self.parse_check_block()?);
            } else {
                items.push(self.parse_data()?);
            }
        }

        Ok(ModuleAst { items, checks })
    }
    ...
    fn parse_type_ref(&mut self) -> Result<TypeRef, ParseErrors> {
        ...
        if items.len() == 1 {
            Ok(items.pop().expect("type item exists"))
        } else {
    ...
    fn parse_type_atom(&mut self) -> Result<TypeRef, ParseErrors> {
        ...
        let ty = match name.as_str() {
            ...
            "any" => TypeRef::Any,
            _ => TypeRef::Named(TypeName::Local(name)),
        };
    ...
    fn parse_expr(&mut self) -> Result<Expr, ParseErrors> {
        ...
        match token.kind {
            ...
            TokenKind::Ident(_) | TokenKind::Any | TokenKind::None => self.parse_ref_or_path(),
    ...
    fn parse_ref_or_path(&mut self) -> Result<Expr, ParseErrors> {
        let start = self.peek().span.start;
        ...
        loop {
            if self.eat(&TokenKind::Slash).is_some() {
                ...
                has_slash = true;
                parts.push(RawPathPart::Slash(part));
        ...
        if has_slash {
            return self.finish_qualified_ref(start.min(first_span.start), first, parts);
        }
        if !has_index && self.at(&TokenKind::LBrace) {
            let ty = match parts.as_slice() {
                [] => TypeName::Local(first),
                _ => return self.err("typed object may only use a bare type name"),
            };
        ...
        let kind = if has_index || parts.len() > 1 {
            ...
            match parts.pop() {
                ...
                Some(RawPathPart::Slash(_)) => unreachable!("slash handled earlier"),
    ...
    fn finish_qualified_ref(
        &self,
        start: usize,
        first: String,
        parts: Vec<RawPathPart>,
    ) -> Result<Expr, ParseErrors> {
        let Some((last, module_parts)) = parts.split_last() else {
            return self.err("qualified data reference must include a name");
        };
        let RawPathPart::Field(name) = last else {
            return self.err("qualified data reference must use the last dot before the name");
        };
        let mut module_id = first;
        for part in module_parts {
            match part {
                RawPathPart::Slash(value) => {
                    module_id.push('/');
                    module_id.push_str(value);
                }
                RawPathPart::Field(_) => {
                    return self.err("qualified data reference may only contain the final dot");
                }
                RawPathPart::Index(_) => {
                    return self.err("qualified data reference cannot contain indexes");
                }
            }
        }
        Ok(Expr {
            kind: ExprKind::QualifiedRef {
                module_id,
                name: name.clone(),
            },
...
fn path_segments(parts: Vec<RawPathPart>) -> Vec<PathSegment> {
    parts
        ...
        .filter_map(|part| match part {
            RawPathPart::Field(part) => Some(PathSegment::Field(part)),
            RawPathPart::Index(index) => Some(PathSegment::Index(index)),
            RawPathPart::Slash(_) => None,
        })
        .collect()
}
