...
/// 严格解析与类型转换共用 Engine 的入口，项目层只补充来源和展示信息。
pub fn parse_records(
    ...
    if !errors.is_empty() {
        return Err(lower::syntax_diagnostics(errors));
    }
    lower::lower_records(schema, &syntax)
}

/// 保留可修复记录和全部诊断，供运行时构建与编辑器分析共同使用。
...
pub fn analyze(schema: &CftSchema, input: SourceInput) -> (SourceAnalysis, Vec<LoadedRecordDraft>) {
    let (syntax, records, diagnostics) =
        analyze_records(schema, &input.text, CfdParseOptions::default());
    let records = records
        .into_iter()
        .map(|mut record| {
