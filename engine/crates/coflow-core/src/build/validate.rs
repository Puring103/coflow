...
use crate::schema::{CftField, CftValueType, TypeName};
...
impl<'s, 'schema> Validator<'s, 'schema> {
    ...
    pub(super) fn validate_record(
        ...
        let is_data = actual_type_meta.kind == coflow_language::cft::syntax::ast::TypeKind::Data;
        if (expected_type.is_none() && is_data) || (expected_type.is_some() && !is_data) {
            self.push(
                CfdDiagnostic::error(
                    CfdErrorCode::TypeMismatch,
                    "table/singleton require top-level records; inline objects require data",
                )
                .with_primary(record, path),
            );
            return None;
    ...
    fn validate_value_inner(
        ...
        match (ty, value) {
            ...
            (CftValueType::Int, LoadedValueDraft::Int(value)) => {
                self.validate_materialized_value(ty, &CfdValue::Int(*value), record, path)?;
                Some(ValueDraft::Value(CfdValue::Int(*value)))
            }
            (CftValueType::Float, LoadedValueDraft::Float(value)) => {
                let value = CfdValue::Float(f64::from(*value as f32));
            ...
            (CftValueType::FString, LoadedValueDraft::FormattedString(value)) => {
            ...
            (CftValueType::RecordRef(expected), LoadedValueDraft::RecordRef(key)) => {
                let (lookup, key) = key
                    .rsplit_once("::")
                    .unwrap_or((expected.as_str(), key.as_str()));
                let lookup = TypeName::new(lookup).ok()?;
...
fn input_value_kind(value: &LoadedValueDraft) -> &'static str {
    match value {
        ...
        LoadedValueDraft::Function(_) => "function",
