...
use coflow_language::cft::syntax::ast::{TypeRef, TypeRefKind};
...
impl ResolvedTypes<'_> {
    ...
    fn resolve_alias_target(
        ...
        match &ty.kind {
            ...
            TypeRefKind::FString => InferredType::Value(CftValueType::FString),
            TypeRefKind::Named(name) => {
                let resolved_name = name.clone();
                match self.symbols.get(&resolved_name).map(|symbol| symbol.kind) {
                    Some(SymbolKind::Type) => self.named_value_type(&resolved_name),
            ...
            TypeRefKind::Dict(key, value) => {
                ...
                if !key_ty.is_unknown() && !is_valid_dict_key(&key_ty) {
                    self.push_diag(
                        ...
                        module,
                        key.span,
                        "dict key type must be string, int, bool, or enum",
                    );
                }
                InferredType::dict(key_ty, self.resolve_alias_target(module, value, visiting))
            }
            TypeRefKind::Option(inner) => {
                let resolved = self.resolve_alias_target(module, inner, visiting);
                if matches!(resolved.value_type(), Some(CftValueType::Option(_))) {
                    self.push_diag(
                        CftErrorCode::InvalidTypeReference,
                        module,
                        ty.span,
                        "optional types cannot be nested",
                    );
                    InferredType::Unknown
                } else {
                    InferredType::option(resolved)
                }
            }
    ...
    pub(super) fn validate_type_headers(&mut self) {
        ...
        for info in self.types.values() {
            use coflow_language::cft::syntax::ast::TypeKind;
            let invalid_parent = info
                .def
                .parent
                .as_ref()
                .and_then(|parent| self.types.get(&parent.name))
                .is_some_and(|parent| match info.def.kind {
                    TypeKind::Data => parent.def.kind != TypeKind::Data,
                    TypeKind::Table | TypeKind::Singleton => parent.def.kind != TypeKind::Table,
                });
            if invalid_parent || (info.def.kind == TypeKind::Singleton && info.def.is_abstract) {
                diagnostics.push(CftDiagnostic::error(
                    CftErrorCode::InvalidAnnotatedFieldType,
                    info.module.clone(),
                    info.def.span,
                    "invalid declaration kind or inheritance relationship",
                ));
            }
            if info.def.kind != TypeKind::Table && has_annotation(&info.def.annotations, "idAsEnum")
            {
                diagnostics.push(CftDiagnostic::error(
                    CftErrorCode::InvalidAnnotatedFieldType,
                    info.module.clone(),
                    info.def.span,
                    "@idAsEnum requires table",
                ));
            }
            for field in &info.def.fields {
                if field.name == "id" && info.def.kind != TypeKind::Data {
                    diagnostics.push(CftDiagnostic::error(
                        CftErrorCode::ReservedIdentifier,
                        info.module.clone(),
                        field.span,
                        "record id is provided by the runtime",
                    ));
                }
                if info.def.kind == TypeKind::Data
                    && (has_annotation(&field.annotations, "localized")
                        || has_annotation(&field.annotations, "dimension"))
                {
                    diagnostics.push(CftDiagnostic::error(
                        CftErrorCode::InvalidAnnotatedFieldType,
                        info.module.clone(),
                        field.span,
                        "dimension fields require a record type",
                    ));
                }
            }
            ...
            .map(|(owner, fields)| {
                let edges = fields
                    ...
                    .filter_map(|(name, field)| match &field.inferred_type {
                        InferredType::Value(CftValueType::Object(target)) => Some(Edge {
                            field: name.clone(),
        ...
        match &ty.kind {
            ...
            TypeRefKind::Named(name) => {
                ...
                match self.symbols.get(&name) {
                    Some(symbol) if symbol.kind == SymbolKind::Type => self.named_value_type(&name),
        ...
        match &ty.kind {
            ...
            TypeRefKind::Named(name) => {
                ...
                match self.symbols.get(&resolved_name) {
                    Some(symbol) if symbol.kind == SymbolKind::Type => {
                        self.named_value_type(&resolved_name)
                    }
                    Some(symbol) if symbol.kind == SymbolKind::Enum => {
                        InferredType::enum_value(crate::EnumName::from_validated(resolved_name))
                    }
            ...
            TypeRefKind::Dict(key, value) => {
                ...
                if !is_valid_dict_key(&key_ty) {
                    diagnostics.push(diagnostic(
                        CftErrorCode::InvalidDictKeyType,
                        key.span,
                        "dict key type must be string, int, bool, or enum".into(),
                    ));
                }
                InferredType::dict(key_ty, self.validate_field_type(module, value, diagnostics))
            }
            TypeRefKind::Option(inner) => {
                let resolved = self.validate_field_type(module, inner, diagnostics);
                if matches!(resolved.value_type(), Some(CftValueType::Option(_))) {
                    diagnostics.push(diagnostic(
                        CftErrorCode::InvalidTypeReference,
                        inner.span,
                        "optional types cannot be nested".into(),
                    ));
                    InferredType::Unknown
                } else {
                    InferredType::option(resolved)
    ...
    fn named_value_type(&self, name: &str) -> InferredType {
        let name_value = crate::TypeName::from_validated(name.to_string());
        if self
            .types
            .get(name)
            .is_some_and(|info| info.def.kind == coflow_language::cft::syntax::ast::TypeKind::Data)
        {
            InferredType::object(name_value)
        } else {
            InferredType::Value(CftValueType::RecordRef(name_value))
        }
    }
}
