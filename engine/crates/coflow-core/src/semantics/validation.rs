...
fn validate_object_type_assignable_at(
    ...
    if schema_type.kind != coflow_language::cft::syntax::ast::TypeKind::Data {
        return Err(CfdValueSemanticError::new(
            CfdValueSemanticErrorKind::RecordType,
            path,
            format!("record type `{actual_type}` cannot be constructed inline"),
        ));
    }
...
fn validate_value_inner<C: CfdValueSemanticContext>(
    ...
    match expected {
        CftValueType::Int => match value {
            CfdValue::Int(value) if i32::try_from(*value).is_ok() => Ok(()),
        ...
        CftValueType::FString => match value {
            CfdValue::FormattedString(_) => Ok(()),
            _ => Err(type_mismatch("fstring", value, path)),
        },
...
fn validate_dict_key(
    ...
    match (expected, value) {
        ...
        | (CftValueType::Bool, CfdDictKey::Bool(_)) => Ok(()),
        (CftValueType::Int, CfdDictKey::Int(value)) if i32::try_from(*value).is_ok() => Ok(()),
