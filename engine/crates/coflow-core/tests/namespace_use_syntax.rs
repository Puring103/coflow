...
use coflow_core::schema::{build_schema, parse_modules, CftFile, CftValueType, ModuleId, TypeName};
...
fn imported_types_resolve_to_qualified_contract_names() {
    ...
    let schema = build_schema(&modules).expect("namespace imports");
    assert_eq!(
        schema
...
fn same_short_names_in_distinct_namespaces_are_independent() {
    ...
    let schema = build_schema(&modules).expect("independent names");
