...
fn build_model(builder: coflow_core::CfdModelBuilder<'_>) -> Result<CfdDataModel, String> {
    builder
        .build()
...
fn error_code_cases() -> Vec<ErrorCodeCase> {
    vec![
        ...
        ErrorCodeCase {
            ...
            schema_source: "data Item { name: string; }",
            declared_type: "Item",
        ...
        ErrorCodeCase {
            ...
            schema_source: "data Stats { hp: int; } data Zone { stats: Stats; }",
            declared_type: "Zone",
        ...
        ErrorCodeCase {
            ...
            schema_source:
                "abstract data Reward {} data CoinReward : Reward { amount: int; } data Item { name: string; }",
        ...
        ErrorCodeCase {
            ...
            schema_source: "abstract data Reward {} data CoinReward : Reward { amount: int; }",
            declared_type: "Reward",
            invalid_text: "Reward{}",
            adjacent_valid_declared_type: "Reward",
            adjacent_valid_text: "CoinReward{amount: 1}",
        ...
        ErrorCodeCase {
            ...
            schema_source: "data Stats { hp: int; attack: int; }",
            declared_type: "Stats",
            invalid_text: "hp: 1, 2",
        ...
        ErrorCodeCase {
            ...
            schema_source: "table Item { name: string; }",
            declared_type: "Item",
            invalid_text: "item_1",
            adjacent_valid_declared_type: "Item",
...
#[test]
fn strings_and_templates_remain_distinct_cell_values() -> TestResult {
    let schema = compile_schema("table Item { name: string; }")?;
    assert!(
        matches!(parse_value(&schema, "string", r#""{self.name}""#)?, LoadedValueDraft::String(value) if value == "{self.name}")
    );
    let source = r#"f"{self.name} {&Item::sword.name}""#;
    assert!(
        matches!(parse_value(&schema, "fstring", source)?, LoadedValueDraft::FormattedString(value) if value.source == source)
    );
    Ok(())
}
...
fn flag_enum_dict_keys_keep_single_variant_identity() -> TestResult {
    let schema = compile_schema(
        r#"
            ...
            table Role { limits: {Permission: int}; }
        "#,
    )?;
    let mut builder = CfdDataModel::builder(&schema);
    builder.add_record(
...
fn rejects_invalid_dict_entries_and_key_types() -> TestResult {
    ...
    assert_has_code(&invalid_key_type, CellValueErrorCode::InvalidDeclaredType);
...
#[test]
fn unknown_declared_types_are_rejected() -> TestResult {
    let schema = compile_schema(
        r#"
...
#[test]
fn float_cells_accept_infinity_but_reject_nonliteral_names() -> TestResult {
    let schema = compile_schema("")?;
    ...
    for text in ["inf", "-inf", "1e999"] {
        assert!(
            matches!(parse_value(&schema, "float", text)?, LoadedValueDraft::Float(v) if v.is_infinite())
        );
    }
    Ok(())
}
...
fn object_type_markers_require_identifier_names_and_closed_boundaries() -> TestResult {
    let schema = compile_schema(
        r#"
            abstract data Reward { key: string; }
            data CurrencyReward : Reward { amount: int; }
        "#,
    )?;
...
fn renders_runtime_values_as_parseable_table_cell_text() -> TestResult {
    ...
    assert_eq!(
        ...
        LoadedValueDraft::record_ref("Item::sword_01")
    );
    ...
    assert_eq!(
        ...
        LoadedValueDraft::OptionSome(Box::new(LoadedValueDraft::record_ref("Item::sword_01")))
    );
...
fn ref_cells_parse_direct_record_ref_shorthand_from_expected_type() -> TestResult {
    ...
    assert_eq!(
        ...
        LoadedValueDraft::Array(vec![
            LoadedValueDraft::record_ref("Item::item_1"),
            LoadedValueDraft::record_ref("Item::item_2"),
        ])
    );
    assert_eq!(
    ...
    assert_eq!(
        ...
        LoadedValueDraft::record_ref("Reward::reward_1")
    );
    assert_eq!(
        parse_value(&schema, "InlineReward", "item: &item_1")?,
        LoadedValueDraft::object_with_declared_type([(
            "item",
            LoadedValueDraft::record_ref("Item::item_1"),
        )])
    ...
    assert_has_code(
        &parse_err(&schema, "Item", "Item{name: Sword}")?,
