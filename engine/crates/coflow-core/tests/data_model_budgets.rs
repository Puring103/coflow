...
fn materialized_node_limit_reports_the_first_rejected_array_item() {
    let schema = compile_schema("table Item { nums: [int]; }");
    let build = |max_nodes| {
        let mut builder =
...
#[test]
fn explicit_default_subtrees_obey_materialized_node_budget() {
    let schema = compile_schema(
        "data Leaf { a: int = 1; b: int = 2; } table Root { left: Leaf = Leaf {}; right: Leaf = Leaf {}; }",
    );
    let build = |max_nodes| {
        let mut builder =
            CfdDataModel::builder(&schema).with_structural_limits(limits(10, max_nodes, 100));
    ...
    assert_eq!(
        diagnostic.message,
        "data value exceeds structural nodes limit 6 (observed 7)"
    );
    assert_eq!(
        diagnostic.primary.as_ref().map(|label| &label.path),
        Some(&CfdPath::root().field("right").field("b"))
    );
}
