...
fn load_example_project() {
    ...
    let manifest = std::env::var("CARGO_MANIFEST_DIR").unwrap();
    let rpg_yaml = PathBuf::from(manifest)
        .parent()
        .unwrap()
        .parent()
        .unwrap()
    ...
    let xlsx_recs = store
        ...
        .unwrap();
    println!(
        "rpg.xlsx types={:?} records={}",
        xlsx_recs.type_names,
        xlsx_recs.records.len()
    );
    if let Some(r) = xlsx_recs.records.first() {
        println!(
            "  first record key={} type={} fields={}",
            r.key,
            r.actual_type,
            r.fields.len()
        );
    ...
    let xlsx_graph = store
        .get_graph(rpg_snap.session_id, "data/rpg.xlsx")
        .unwrap();
    println!(
        "rpg.xlsx graph nodes={} edges={}",
        xlsx_graph.nodes.len(),
        xlsx_graph.edges.len()
    );
    ...
    let prog = store
        ...
        .unwrap();
    let stage_count = prog
        .records
        .iter()
        .filter(|r| r.actual_type == "Stage")
        .count();
    assert_eq!(
        stage_count, 3,
        "expected 3 Stage records in progression.cfd, got {stage_count}"
    );

    // Inspect the first Item record to confirm featured_stage resolved
    let xlsx_recs2 = store
        .get_file_records(rpg_snap.session_id, "data/rpg.xlsx")
        .unwrap();
    let healing = xlsx_recs2
        .records
        .iter()
        .find(|r| r.key == "healing_potion")
        .unwrap();
    let fs = healing
        .fields
        .iter()
        .find(|f| f.name == "featured_stage")
        .unwrap();
