use coflow_core::{
    ...
    vm::{compiler::{compile, CompileContext}, executor::ExecutionLimits},
};
use std::sync::Arc;

fn run(body: &str) -> i32 {
    let source = format!("data Point {{ x: int; y: int = 2; read: fn() -> int => {{ self.x + self.y }}; }} table Rule {{ run: fn() -> int => {{ {body} }}; }}");
    let schema = build_schema(&parse_modules([CftFile::from_source(ModuleId::from("builders"), source)])).unwrap();
    ...
    for profile in [OptimizationProfile::Debug, OptimizationProfile::Release] {
        ...
        let runtime = result.runtime.unwrap_or_else(|error| panic!("{error}: {:?}", result.diagnostics));
        let function = runtime.field(runtime.record("Rule", "a").unwrap(), "run").unwrap();
        let HostValue::Int(value) = runtime.invoke(function, &[], ExecutionLimits::default()).unwrap() else { panic!("expected int"); };
        runtime.collect().unwrap();
        assert_eq!(runtime.dynamic_value_count().unwrap(), 0, "无外部返回根时构造缓冲与 self 环应回收");
        if let Some(previous) = returned { assert_eq!(previous, value); }
        returned = Some(value);
    }
    returned.unwrap()
}
...
fn rejects_aliases_captures_uninitialized_fields_and_nested_mutation() {
    let schema = build_schema(&parse_modules([CftFile::from_source(ModuleId::from("invalid"), "data Point { x: int; y: int = 0; } table Item { x: int; }")])).unwrap();
    for source in [
...
fn control_transfer_discards_unfrozen_builders() {
    ...
    assert_eq!(run("var ignored: Point = build Point as b { return 13; }; ignored.x"), 13);
}

#[test]
fn unpublished_self_bindings_cannot_escape_or_execute() {
    let schema = build_schema(&parse_modules([CftFile::from_source(ModuleId::from("invalid"), "data Point { x: int = 1; read: fn() -> int => { self.x }; }")])).unwrap();
    for source in [
