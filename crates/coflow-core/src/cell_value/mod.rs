...
fn parse_value(
    ...
    match ty {
        ...
        CellType::Int => Ok(LoadedValueDraft::Int(i64::from({
            ...
            .map_err(|_| type_mismatch("int"))?;
            text.replace('_', "")
                .parse::<i32>()
                .map_err(|_| type_mismatch("int"))?
        }))),
        CellType::Float => Ok(LoadedValueDraft::Float(f64::from(
            coflow_language::lexical::parse_float_literal(text)
                .map_err(|_| type_mismatch("float"))?,
        ))),
        ...
        CellType::FString => parse_automatic_formatted_string(text)?
            .map(LoadedValueDraft::FormattedString)
            .ok_or_else(|| type_mismatch("fstring literal")),
