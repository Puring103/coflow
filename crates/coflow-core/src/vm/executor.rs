...
impl Slot {
    ...
    pub(crate) fn scalar_id(self) -> Option<u64> {
        let (tag, bits) = match self {
            Self::None | Self::Unit => (1, 0), Self::Bool(value) => (2, u64::from(value)),
            Self::Int(value) => (3, u64::from(value as u32)), Self::Float(value) => (4, u64::from(value.to_bits())),
            _ => return None,
        };
        Some((tag << 44) | bits)
    }
    pub(crate) fn from_scalar_id(id: u64) -> Option<Self> {
        ...
        if bits > u64::from(u32::MAX) { return None; }
        match id >> 44 {
            1 if bits == 0 => Some(Self::None), 2 if bits <= 1 => Some(Self::Bool(bits != 0)),
            3 => Some(Self::Int(bits as u32 as i32)), 4 => Some(Self::Float(f32::from_bits(bits as u32))),
            _ => None,
        }
    }
    pub(crate) fn handle(id: u64) -> Self {
        assert!(id <= Self::MAX_HANDLE, "执行引用必须在发布前验证");
        Self::Handle(HandleId([id as u16, (id as u64 >> 16) as u16, (id as u64 >> 32) as u16]))
    }
}
...
pub(crate) trait ExecutionHost {
    ...
    fn build(&self, _operation: &super::construction::BuildOp<Slot>, _ty: &crate::schema::CftValueType) -> Result<Slot, ExecutionError> {
...
impl Drop for HostBudgetGuard {
    fn drop(&mut self) { self.0.leave(0); }
}
/// 同步重入与递归导入共享临时缓冲占用；归还只撤销本次预留，不重置外层预算。
#[derive(Debug)]
pub(crate) struct TemporaryBytes { budget: Budget, bytes: usize }
impl TemporaryBytes {
    pub(crate) fn resize(&mut self, bytes: usize, heap_bytes: usize) -> Result<(), ExecutionError> {
        if bytes > self.bytes {
            let extra = bytes - self.bytes;
            if extra > self.budget.max_heap_bytes().saturating_sub(heap_bytes) { return Err(error("动态内存预算耗尽")); }
            self.budget.usage.temporary_bytes.set(self.budget.usage.temporary_bytes.get() + extra);
        } else { self.budget.usage.temporary_bytes.set(self.budget.usage.temporary_bytes.get() - (self.bytes - bytes)); }
        self.bytes = bytes; Ok(())
    }
}
impl Drop for TemporaryBytes {
    fn drop(&mut self) {
        ...
        usage.temporary_bytes.set(usage.temporary_bytes.get() - self.bytes);
    }
}
impl Budget {
    pub(crate) fn reserve_temporary(&self, bytes: usize, heap_bytes: usize) -> Result<TemporaryBytes, ExecutionError> {
        if bytes > self.max_heap_bytes().saturating_sub(heap_bytes) { return Err(error("动态内存预算耗尽")); }
        self.usage.temporary_bytes.set(self.usage.temporary_bytes.get() + bytes);
        Ok(TemporaryBytes { budget: self.clone(), bytes })
    }
    ...
    pub fn max_heap_bytes(&self) -> usize {
        self.limits.max_heap_bytes.saturating_sub(self.usage.temporary_bytes.get())
    }
...
struct Frame {
    ...
    builders: Vec<Slot>,
    builders_memory: TemporaryBytes,
}

pub(crate) fn execute<H: ExecutionHost>(
    host: &H,
    binding: Binding,
    arguments: &[Slot],
    budget: Budget,
...
impl<H: ExecutionHost> Drop for Execution<'_, H> {
    fn drop(&mut self) {
        for frame in &self.frames {
            for builder in &frame.builders {
                let _ = self.host.build(&super::construction::BuildOp::Drop { builder: *builder }, &crate::schema::CftValueType::Unit);
            }
...
impl<H: ExecutionHost> Execution<'_, H> {
    fn reserve_buffers(&mut self, registers: usize, frames: usize, roots: usize, operands: usize) -> Result<(), ExecutionError> {
        let capacity = |old: usize, required: usize| if required <= old { old } else { old.saturating_mul(2).max(required).max(4) };
        ...
        let bytes = registers.checked_add(roots).and_then(|n| n.checked_add(operands)).and_then(|n| n.checked_mul(size_of::<Slot>()))
            .and_then(|n| frames.checked_mul(size_of::<Frame>()).and_then(|frames| n.checked_add(frames))).ok_or_else(|| error("执行缓冲容量溢出"))?;
        // 所有执行缓冲容量统一计入共享预算，暂停帧与重入执行器的容量同时有效。
        self.buffers_memory.resize(bytes, self.host.heap_bytes())?;
        self.registers.try_reserve_exact(registers - self.registers.len()).map_err(|_| error("寄存器分配失败"))?;
        self.frames.try_reserve_exact(frames - self.frames.len()).map_err(|_| error("调用帧分配失败"))?;
        self.roots_scratch.try_reserve_exact(roots - self.roots_scratch.len()).map_err(|_| error("根缓冲分配失败"))?;
        self.operands_scratch.try_reserve_exact(operands - self.operands_scratch.len()).map_err(|_| error("操作数缓冲分配失败"))?;
        Ok(())
    }
    fn reserve_frame(&mut self, registers: usize) -> Result<(), ExecutionError> {
        ...
        if let Err(cause) = self.reserve_buffers(required, self.frames.len().saturating_add(1), self.roots_scratch.len(), self.operands_scratch.len()) {
            self.budget.leave(registers); return Err(cause);
        }
        Ok(())
    }
    fn push(
        ...
        self.frames.push(Frame {
            ...
            destination,
            builders: Vec::new(),
            builders_memory: self.budget.reserve_temporary(0, self.host.heap_bytes())?,
        });
        Ok(())
    }
    ...
    fn push_window(&mut self, binding: Binding, start: usize, count: usize, destination: usize) -> Result<(), ExecutionError> {
        if count != binding.program.parameters.len() || binding.captures.len() != binding.program.captures.len() {
            return Err(error("调用签名或捕获布局不匹配"));
        }
        let frame = self.frames.last().ok_or_else(|| error("调用栈为空"))?;
        let tail = frame.builders.is_empty() && frame.binding.program.is_tail_call(frame.pc.saturating_sub(1));
        if tail {
            let base = frame.base;
            let old = frame.binding.program.registers.len();
            let new = binding.program.registers.len();
            let remaining = self.budget.limits.max_registers.saturating_sub(self.budget.usage.registers.get() - old);
            if new > remaining { return Err(error("调用深度或寄存器预算耗尽")); }
            if new > old { self.reserve_buffers(base.saturating_add(new), self.frames.len(), self.roots_scratch.len(), self.operands_scratch.len())?; }
            ...
            self.budget.usage.registers.set(self.budget.usage.registers.get() - old + new);
            let frame = self.frames.last_mut().ok_or_else(|| error("调用栈为空"))?;
            frame.binding = binding; frame.pc = 0;
            return Ok(());
        }
        self.reserve_frame(binding.program.registers.len())?;
        let base = self.registers.len();
        self.registers.resize(base + binding.program.registers.len(), Slot::Empty);
        // 源窗口属于暂停的调用帧；扩容后使用索引重新定位，不持有可能失效的指针。
        self.registers.copy_within(start..start + count, base);
        self.frames.push(Frame { binding, pc: 0, base, destination: Some(destination), builders: Vec::new(), builders_memory: self.budget.reserve_temporary(0, self.host.heap_bytes())? });
        Ok(())
    }
    ...
    fn values(&self, base: usize, registers: &[Register]) -> Result<(Vec<Slot>, TemporaryBytes), ExecutionError> {
        let bytes = registers.len().checked_mul(size_of::<Slot>()).ok_or_else(|| error("操作数缓冲超限"))?;
        let memory = self.budget.reserve_temporary(bytes, self.host.heap_bytes())?;
        let mut values = Vec::new();
        values.try_reserve_exact(registers.len()).map_err(|_| error("操作数缓冲分配失败"))?;
        for register in registers { values.push(self.get(base, *register)?); }
        Ok((values, memory))
    }
    fn gather_operands(
        &mut self,
        ...
        self.reserve_buffers(self.registers.len(), self.frames.len(), self.roots_scratch.len(), registers.len())?;
    ...
    /// 克隆当前帧程序句柄；仅在需要读取附表的指令上支付两次原子引用计数。
    fn program(&self) -> Result<Arc<super::image::ValidatedProgram>, ExecutionError> {
        Ok(self
            .frames
            .last()
    ...
    fn publish_roots(&mut self) -> Result<(), ExecutionError> {
        if !self.host.needs_roots() { return Ok(()); }
        self.publish_roots_force()
    }
    fn publish_roots_force(&mut self) -> Result<(), ExecutionError> {
        let required = self.frames.iter().try_fold(self.registers.len(), |n, frame| n.checked_add(frame.binding.captures.len()).and_then(|n| n.checked_add(1))).ok_or_else(|| error("根缓冲容量溢出"))?;
        self.reserve_buffers(self.registers.len(), self.frames.len(), required, self.operands_scratch.len())?;
    ...
    fn run(&mut self) -> Result<Slot, ExecutionError> {
        loop {
            // 取指：只借用调用帧读取指令并推进 pc，热路径上不克隆任何 Arc。
            let (base, owner, instruction) = {
                let frame = self.frames.last_mut().ok_or_else(|| error("调用栈为空"))?;
                ...
                frame.pc += 1;
                (frame.base, frame.binding.owner, instruction)
            };
            ...
            let value = match opcode {
                Opcode::Build => {
                    ...
                    let operation = program.builders[index].map(|r| self.get(base, r).map_err(|e| e.to_string())).map_err(|message| error(&message))?;
                    use super::construction::BuildOp as B;
                    if matches!(operation, B::Start { .. }) {
                        let frame = self.frames.last_mut().ok_or_else(|| error("构造操作缺少调用帧"))?;
                        if frame.builders.len() == frame.builders.capacity() {
                            let capacity = frame.builders.capacity().saturating_mul(2).max(4);
                            frame.builders_memory.resize(capacity.checked_mul(size_of::<Slot>()).ok_or_else(|| error("构造根容量溢出"))?, self.host.heap_bytes())?;
                            frame.builders.try_reserve_exact(capacity - frame.builders.len()).map_err(|_| error("构造根分配失败"))?;
                        }
                    }
                    let result = self.host.build(&operation, &program.registers[instruction.a() as usize])?;
                    let frame = self.frames.last_mut().ok_or_else(|| error("构造操作缺少调用帧"))?;
                    match operation {
                        B::Start { .. } => frame.builders.push(result),
                        B::Freeze { builder } | B::Drop { builder } => frame.builders.retain(|value| *value != builder),
                        _ => {}
                    }
                    result
                }
                ...
                Opcode::Return => {
                    ...
                    for builder in &frame.builders {
                        self.host.build(&super::construction::BuildOp::Drop { builder: *builder }, &crate::schema::CftValueType::Unit)?;
                    }
                    self.budget.leave(frame.binding.program.registers.len());
                ...
                Opcode::Call | Opcode::CallDirect => {
                    ...
                    let (target, arguments_start, arguments_len) = {
                        ...
                        let (target, arguments_start, arguments_len) = if opcode == Opcode::CallDirect {
                            let site = &program.direct_calls[index];
                            (Callable::Program(self.host.direct_callable(site.function)?), site.arguments_start, site.arguments_len)
                        } else {
                            let site = &program.calls[index];
                            let target = self.get(base, site.target)?;
                            (self.host.callable(target)?, site.arguments_start, site.arguments_len)
                        };
                        let arguments = program
                            .operands(arguments_start, arguments_len)
                            .ok_or_else(|| error("调用操作数范围越界"))?;
                        (target, base + arguments.first().copied().map_or(0, usize::from), arguments.len())
                    };
                    match target {
                        Callable::Program(binding) => {
                            self.push_window(binding, arguments_start, arguments_len, destination)?;
                            continue;
                        }
                        Callable::Host(target) => {
                            ...
                            self.publish_roots_force()?;
                            self.host.call_host(target, &self.registers[arguments_start..arguments_start + arguments_len])?
                        }
                    }
                }
                Opcode::Closure => {
                    ...
                    self.host.closure(
                        Binding {
                            program: program.closure(index).ok_or_else(|| error("闭包程序越界"))?,
                            owner: closure_owner,
                ...
                Opcode::Format => {
                    self.publish_roots()?;
                    let program = self.program()?;
                    let plan = program.formats.get(index).ok_or_else(|| error("格式计划越界"))?;
                    self.host.format(plan, &self.registers[base..base + program.registers.len()])?
                }
                ...
                Opcode::ForPrep => {
                    ...
                    if exhausted {
                        let target = site.target as usize;
                        if target >= length {
                            return Err(error("跳转越界"));
                        }
                        if let Some(frame) = self.frames.last_mut() {
                ...
                Opcode::ForLoop => {
                    ...
                    if !exhausted {
                        let next = value.checked_add(1).ok_or_else(|| error("区间自增溢出"))?;
...
fn int_binary(op: u8, left: i32, right: i32) -> Result<Slot, ExecutionError> {
    ...
    let value = match op {
        ...
        6 => u32::try_from(right).ok().and_then(|right| left.checked_pow(right)),
...
pub(crate) fn scalar_binary(op: u8, left: Slot, right: Slot) -> Option<Result<Slot, ExecutionError>> {
...
mod compact_slot_tests {
    ...
    fn temporary_reservations_accumulate_across_nested_budgets_and_unwind() {
        let budget = Budget::new(ExecutionLimits { max_heap_bytes: 100, ..ExecutionLimits::default() });
        ...
        drop(inner); assert_eq!(budget.max_heap_bytes(), 60);
        drop(outer); assert_eq!(budget.max_heap_bytes(), 100);
    }
    #[test]
    fn slots_are_eight_bytes_and_references_round_trip_without_losing_float_bits() {
        ...
        for id in [0, 1, 65535, 65536, u64::from(u32::MAX), Slot::MAX_HANDLE] {
            let Slot::Handle(handle) = Slot::handle(id) else { unreachable!(); };
            assert_eq!(handle.get(), id);
        }
        for bits in [0, 0x80000000, 0x7fc00001, 0xff800000] {
            let Slot::Float(value) = Slot::Float(f32::from_bits(bits)) else { unreachable!(); };
