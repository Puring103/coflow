...
use crate::schema::{CftSchema, CftValueType, FieldName, RecordKey, TypeName, VariantName};
use crate::semantics::{
...
impl<'a> ModelCompiler<'a> {
    ...
    pub(crate) fn build(mut self) -> Result<CfdModelBuildOutput, CfdDiagnostics> {
        let mut drafts = self.validate_input_records();
        // Host 身份由契约创建，数据由绑定读取；用户 CFD 仍不能创建服务记录。
        for ty in self.schema.singleton_types().filter(|ty| ty.is_host) {
            drafts.push(RecordDraft {
                key: ty
                    .name
                    .rsplit("::")
                    .next()
                    .unwrap_or(ty.name.as_str())
                    .to_string(),
                actual_type: ty.name.clone(),
                fields: BTreeMap::new(),
        ...
        self.move_editable_diagnostics(|code| {
            matches!(
                code,
                crate::CfdErrorCode::SingletonRecordCountInvalid
                    | crate::CfdErrorCode::SingletonKeyMissingOrInvalid
                    | crate::CfdErrorCode::SingletonKeyCollision
            )
        });
        self.fail_if_diagnostics()?;
...
pub(super) const fn semantic_error_code(kind: CfdValueSemanticErrorKind) -> crate::CfdErrorCode {
    match kind {
        ...
        CfdValueSemanticErrorKind::RecordType | CfdValueSemanticErrorKind::TypeMismatch => {
            crate::CfdErrorCode::TypeMismatch
        }
