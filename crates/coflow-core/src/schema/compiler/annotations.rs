...
use coflow_language::cft::syntax::ast::{Annotation, AnnotationArg, FieldDef};
    ...
    pub(super) fn validate_annotations(&mut self) {
            ...
            if let Some(host) = find_annotation(&info.def.annotations, "Host") {
                    ...
                    || info.def.kind != coflow_language::cft::syntax::ast::TypeKind::Singleton
                {
                    push_diag(
                        ...
                        &info.module,
                        host.span,
                        "@Host requires a concrete singleton declaration",
                    );
                }
            }
        ...
        for info in self.types.values() {
            for field in &info.def.fields {
                ...
                self.validate_field_annotations(&info.module, field, &mut diagnostics);
            }
        }
