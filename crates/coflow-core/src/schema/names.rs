...
fn standard_name(value: &str) -> bool {
    value.split("::").all(is_cft_identifier)
}
...
cft_name!(TypeName, "type name", standard_name);
impl TypeName {
    pub(in crate::schema) fn from_validated(value: impl Into<String>) -> Self {
        Self(value.into())
    }
}
cft_name!(FieldName, "field name", is_cft_identifier, trusted);
...
cft_name!(RecordKey, "record key", is_cft_identifier);
