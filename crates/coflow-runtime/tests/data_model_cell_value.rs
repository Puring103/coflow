...
fn build_model(builder: coflow_runtime::CfdModelBuilder<'_>) -> Result<CfdDataModel, String> {
    builder
        .build()
...
fn declared_error_code_names() -> BTreeSet<String> {
    let source = include_str!("../src/data_model/cell_value/diagnostics.rs");
...
fn parses_formatted_string_cell_references() -> TestResult {
    ...
    assert_eq!(value.segments.len(), 5);
    let references = value
        .segments
        .iter()
        .filter_map(|segment| match segment {
            coflow_runtime::LoadedFormatSegment::Reference(reference) => Some(reference),
            coflow_runtime::LoadedFormatSegment::Text(_) => None,
        })
        .collect::<Vec<_>>();
    assert_eq!(
        (
...
fn renders_runtime_values_as_parseable_table_cell_text() -> TestResult {
    ...
    let stats = CfdValue::Object(Box::new(
        coflow_runtime::CfdObject::try_new(
            "Stats",
