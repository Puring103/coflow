use crate::data_model::{CfdErrorCode, CfdPath, CfdPathSegment, DimensionFieldLookupError};
use coflow_language::{
...
use crate::checker::eval::{EvalValue, ValueLocation};
...
pub struct CheckDiagnostic {
    pub diagnostic: crate::data_model::CfdDiagnostic,
...
pub struct CheckSchemaLocation {
    pub module: coflow_language::ModuleId,
    pub span: coflow_language::Span,
}

impl From<crate::data_model::CfdDiagnostic> for CheckDiagnostic {
    fn from(diagnostic: crate::data_model::CfdDiagnostic) -> Self {
        Self {
            diagnostic,
            contexts: Vec::new(),
    ...
    match stmt {
        ...
        CftSchemaCheckStmt::Quantifier {
            ...
            let binding = match bindings {
                coflow_language::CftSchemaQuantifierBindings::Single { binding } => binding.clone(),
                coflow_language::CftSchemaQuantifierBindings::Array { item, index } => {
                    format!("{item}, {index}")
                }
                coflow_language::CftSchemaQuantifierBindings::Dict { key, value } => {
                    format!("{key}, {value}")
                }
            };
            format!(
...
pub(crate) fn format_value_for_message(value: &EvalValue<'_>) -> String {
    if let Some(scalar) = value.scalar() {
        return crate::checker::eval::format_scalar(
            scalar,
            crate::checker::eval::ScalarFormat::Diagnostic,
        );
    }
    match value {
