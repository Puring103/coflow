...
use crate::checker::diagnostics::dimension_lookup_error_message;
use crate::checker::eval::{EvalRecordRef, EvalValue, LocatedEvalValue, ValueLocation};
use crate::checker::CheckProjection;
...
fn attach_dimension_origin(
    model: &CfdDataModel,
    projection: &CheckProjection,
    label: &mut crate::data_model::CfdLabel,
) {
    ...
    let Some(field) = label
        ...
        .find_map(|segment| match segment {
            crate::data_model::CfdPathSegment::Field(field) => Some(field.as_str()),
            crate::data_model::CfdPathSegment::Index(_)
            | crate::data_model::CfdPathSegment::DictKey(_) => None,
        })
    else {
        return;
    };
...
impl CheckProjectionView {
    ...
    pub(crate) fn materialize<'model>(
        ...
        Ok(Some(MaterializedDimensionValue {
            ...
            location: logical_location.backed_by(crate::checker::eval::ModelCursor::dimension(
                source_record,
                field_name,
                self.variant.as_str(),
            )),
