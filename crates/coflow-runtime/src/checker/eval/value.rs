...
pub(crate) fn model_value<'a>(
    ...
    for segment in &cursor.path.segments {
        value = match (segment, value) {
            ...
            (crate::data_model::CfdPathSegment::Index(index), CfdValue::Array(items)) => {
                items.get(*index)?
            }
            (crate::data_model::CfdPathSegment::DictKey(key), CfdValue::Dict(entries)) => entries
                .iter()
                .find(|(entry_key, _)| crate::data_model::format_cfd_dict_key(entry_key) == *key)
                .map(|(_, value)| value)?,
            _ => return None,
        };
    }
