...
use crate::data_model::semantics::{
...
impl<'a> ModelCompiler<'a> {
    ...
    pub(crate) fn build(mut self) -> Result<CfdModelBuildOutput, CfdDiagnostics> {
        ...
        self.move_editable_diagnostics(|code| {
            matches!(
                code,
                crate::data_model::CfdErrorCode::RefTargetNotFound
                    | crate::data_model::CfdErrorCode::RefTargetTypeMismatch
            )
        });
        self.fail_if_diagnostics()?;
    ...
    fn move_editable_diagnostics(
        &mut self,
        predicate: impl Fn(crate::data_model::CfdErrorCode) -> bool,
    ) {
    ...
    fn validate_dimension_value(
        ...
        let Some(inheritance_root) = self.schema.inheritance_root(input.source_type.as_str())
            ...
            self.diagnostics.push(dimension_diagnostic(
                ...
                None,
                path,
                crate::data_model::CfdErrorCode::UnknownType,
            ...
            .is_assignable(record.actual_type(), input.source_type.as_str())
        {
            self.diagnostics.push(dimension_diagnostic(
                ...
                Some(record_id),
                path,
                crate::data_model::CfdErrorCode::ObjectTypeMismatch,
                format!(
        ...
        let Some(field) = self
            ...
            self.diagnostics.push(dimension_diagnostic(
                ...
                Some(record_id),
                path,
                crate::data_model::CfdErrorCode::UnknownField,
                format!(
        ...
        let Some(binding) = &field.dimension else {
            self.diagnostics.push(dimension_diagnostic(
                ...
                Some(record_id),
                path,
                crate::data_model::CfdErrorCode::TypeMismatch,
                format!(
        ...
        if !seen.insert((record_id, input.field.clone(), input.variant.clone())) {
            self.diagnostics.push(dimension_diagnostic(
                ...
                Some(record_id),
                path,
                crate::data_model::CfdErrorCode::DuplicateId,
                format!(
        ...
        let draft = Validator::new(&self.schema, &mut self.diagnostics, self.structural_limits)
            .validate_value(
                ...
                Some(record_id),
                path.clone(),
                coflow_language::limits::TraversalCursor::root(),
            );
...
fn dimension_diagnostic(
    ...
    path: CfdPath,
    code: crate::data_model::CfdErrorCode,
...
fn validate_resolved_records(
    ...
    for (index, record) in records.iter().enumerate() {
        ...
        for (field_name, value) in record.fields() {
            ...
            if let Err(error) = crate::data_model::semantics::validate_value_for_schema(
                schema.cft(),
                &context,
                request,
            ) {
...
) -> crate::data_model::CfdErrorCode {
    match kind {
        CfdValueSemanticErrorKind::UnknownType => crate::data_model::CfdErrorCode::UnknownType,
        CfdValueSemanticErrorKind::AbstractType => {
            crate::data_model::CfdErrorCode::AbstractRecordType
        }
        ...
        CfdValueSemanticErrorKind::UnknownField => crate::data_model::CfdErrorCode::UnknownField,
