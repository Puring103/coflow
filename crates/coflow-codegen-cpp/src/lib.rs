...
fn render_types(model: &CodegenModel) -> String {
    let mut out = String::from(
        "// Generated by Coflow. Do not edit.\n#pragma once\n\
         ...
         template <typename T> struct CoflowRef { std::string id; const T* value; CoflowRef() : value(0) {} };\n\
         template <typename T> struct Localized { T default_value; };\n\
         template <typename T> struct CoflowTable { std::vector<std::unique_ptr<T> > records; std::map<std::string, T*> by_id; };\n\n",
    );
...
fn render_type(out: &mut String, ty: &CodegenType, model: &CodegenModel) {
    ...
    if ty.parent.is_none() {
        out.push_str("    std::string id;\n");
    }
    if ty.is_abstract || ty.concrete_types.len() > 1 {
...
fn render_json_loader(model: &CodegenModel) -> String {
    let mut out = String::from(
        "// Generated by Coflow. Do not edit.\n#pragma once\n#include \"CoflowTypes.h\"\n\
         ...
         template <typename T> bool Read(const Json& value, CoflowRef<T>& out) { return Read(value, out.id); }\n\
...
fn render_type_reader(out: &mut String, ty: &CodegenType) {
    ...
    if let Some(parent) = &ty.parent {
        ...
        out.push_str("    Json::const_iterator id = value.find(\"id\"); if (id != value.end() && !Read(*id, out.id)) return false;\n");
    }
    for field in &ty.own_fields {
...
template <typename T> bool Read(const WireValue& wire, CoflowRef<T>& value) { return Read(wire, value.id); }
...
fn render_protobuf_type_reader(out: &mut String, ty: &CodegenType, model: &CodegenModel) {
    ...
    if !ty.is_struct {
        out.push_str("    { const WireValue* field = Find(fields, 1); if (!field || !Read(*field, value.id)) return false; }\n");
    }
