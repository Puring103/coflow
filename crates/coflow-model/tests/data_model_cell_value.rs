...
fn build_model(builder: coflow_model::CfdModelBuilder<'_>) -> Result<CfdDataModel, String> {
    builder
        .build()
...
fn parses_formatted_string_cell_references() -> TestResult {
    ...
    let references = value
        ...
        .filter_map(|segment| match segment {
            coflow_model::LoadedFormatSegment::Reference(reference) => Some(reference),
            coflow_model::LoadedFormatSegment::Text(_) => None,
        })
        .collect::<Vec<_>>();
    assert_eq!(
        (
...
fn renders_runtime_values_as_parseable_table_cell_text() -> TestResult {
    ...
    let stats = CfdValue::Object(Box::new(
        coflow_model::CfdObject::try_new(
            "Stats",
