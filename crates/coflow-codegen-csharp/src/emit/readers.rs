...
pub(super) fn read_field_expr(
    ...
    let name = &field.name;
    let reader = read_token_expr(field.value_type.non_nullable(), "token", context, view)?;
...
pub(super) fn read_token_expr(
    ...
    match ty {
        CftValueType::Int => Ok(format!("CoflowJson.ReadInt({token})")),
        CftValueType::Float => Ok(format!("CoflowJson.ReadFloat({token})")),
        CftValueType::Bool => Ok(format!("CoflowJson.ReadBool({token})")),
        CftValueType::String => Ok(format!("CoflowJson.ReadString({token})")),
        ...
        CftValueType::Array(inner) => Ok(format!(
        ...
        CftValueType::Nullable(inner) => Ok(format!(
...
pub(super) fn read_messagepack_field_expr(
    ...
    read_messagepack_expr(&field.value_type, reader, context, view)
}
...
pub(super) fn read_messagepack_expr(
    ...
    match ty {
        CftValueType::Int => Ok(format!("CoflowMessagePack.ReadInt(ref {reader})")),
        CftValueType::Float => Ok(format!("CoflowMessagePack.ReadFloat(ref {reader})")),
        CftValueType::Bool => Ok(format!("CoflowMessagePack.ReadBool(ref {reader})")),
        CftValueType::String => Ok(format!("CoflowMessagePack.ReadString(ref {reader})")),
        CftValueType::Enum(name) if view.is_id_as_enum(name) => Ok(format!(
...
fn read_messagepack_dict_key_expr(
    ...
    match ty.non_nullable() {
        CftValueType::String => Ok(key.to_string()),
        CftValueType::Int => Ok(format!("CoflowMessagePack.ReadIntKey({key})")),
        CftValueType::Enum(name) => Ok(format!(
