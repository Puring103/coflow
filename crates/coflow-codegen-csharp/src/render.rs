...
pub fn render_cfd_loader(
    ...
    output
        .push_str("    public static IReadOnlyList<string> SourceFiles { get; } = new[]\n    {\n");
    ...
    output.push_str("    public static ");
    output.push_str(&project.database_class);
    output.push_str(" Load(ICfdTextLoader loader, CfdLoadOptions? options = null) => LoadDocuments(CfdLoader.LoadDocuments(loader, SourceFiles, options), options);\n\n");
    output.push_str("    public static ");
    output.push_str(&project.database_class);
    ...
    for table in &project.database.tables {
        output.push_str(&format!(
            "        var {}Records = documents.SelectMany(document => document.Records).Where(record => record.DeclaredType == \"{}\").Select(record => Read{}(record, context)).ToList();\n        var {}Index = {}Records.ToDictionary(record => record.{});\n        var {} = new Table<{}, {}>({}Records, {}Index);\n",
    ...
    for singleton in &project.singletons {
        output.push_str(&format!(
            "        var {} = documents.SelectMany(document => document.Records).Where(record => record.DeclaredType == \"{}\").Select(record => Read{}(record, context)).FirstOrDefault()!;\n",
