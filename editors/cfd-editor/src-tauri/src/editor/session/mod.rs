...
fn snapshot_record_before_delete(
    ...
    let record_file_map = session.record_file_map();
    let fields = record_to_field_cells_for_session(record, &session.engine.model, &record_file_map);
    let snapshot = FieldValue::Object {
        actual_type: record.actual_type.clone(),
        fields,
    };
    Ok((Some(snapshot), Some(record.actual_type.clone())))
}
